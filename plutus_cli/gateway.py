...
Handles: plutus gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``plutus update``
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}plutus gateway status\n"
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `plutus gateway`")
...
SERVICE_DESCRIPTION = "plutus-agent Gateway - Messaging Platform Integration"
...
def _profile_arg(hermes_home: str | None = None) -> str:
    ...
    For ``~/.plutus-agent/profiles/<name>``, returns ``"--profile <name>"``.
...
def get_service_name() -> str:
    ...
    Profile ``~/.plutus-agent/profiles/coder`` returns ``hermes-gateway-coder``.
...
def print_legacy_unit_warning() -> None:
    ...
    print_info("    plutus gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: plutus gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo plutus gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    plutus gateway uninstall")
    print_info("    sudo plutus gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            ...
            if run_as_user:
                print_info(f"  After setup, run: sudo plutus gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo plutus gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo plutus gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Profile ``~/.plutus-agent/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    ...
    the calling user's home.  This translates it to the target user's
    equivalent path:
      /root/.plutus-agent                    → /home/alice/.plutus-agent
      /root/.plutus-agent/profiles/coder     → /home/alice/.plutus-agent/profiles/coder
      /opt/custom-hermes                      → /opt/custom-hermes  (kept as-is)

    Uses ``HERMES_HOME_DIR_NAME`` from plutus_constants so this works
    correctly on any fork (``.plutus-agent`` here, ``.hermes`` upstream).
    Previously hardcoded ``.hermes`` which broke installs on this fork —
    ...
    target_default = Path(target_home_dir) / HERMES_HOME_DIR_NAME

    # Default ~/<dir> → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/<dir> → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/<dir>) — keep as-is
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}plutus gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}plutus gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        # unit immediately so `plutus gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}plutus gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}plutus gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}plutus gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}plutus gateway start{scope_flag}")
...
def launchd_stop():
    ...
    # `plutus gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        ...
        print("  Run: plutus gateway start")
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                if access_idx == 0:
                    ...
                    print_info("  Skipped — configure later with 'plutus gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from plutus_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from plutus_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print_info("  3. Hermes will store the returned account_id/token in ~/.plutus-agent/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `plutus gateway setup`.")
        return
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"plutus-agent\"")
...
def gateway_setup():
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: plutus gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  plutus gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: plutus gateway run")
                print_info("  For persistence:   tmux new -s hermes 'plutus gateway run'")
        ...
        print()
        print_info("No platforms configured. Run 'plutus gateway setup' when ready.")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: plutus gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: plutus gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s hermes 'plutus gateway run'")
                print()
            ...
            print()
            print("  plutus gateway run                              # direct foreground")
            print("  tmux new -s hermes 'plutus gateway run'         # persistent via tmux")
            print("  nohup plutus gateway run > ~/.plutus-agent/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: plutus gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: plutus gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: plutus gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: plutus gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: plutus gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  plutus gateway install")
                    print("  sudo plutus gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  plutus gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup plutus gateway run > ~/.plutus-agent/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s hermes 'plutus gateway run'         # persistent via tmux")
                    print("  nohup plutus gateway run > ~/.plutus-agent/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  plutus gateway install  # Install as user service")
                    print("  sudo plutus gateway install --system  # Install as boot-time system service")
