"""
Doctor command for plutus CLI.

Diagnoses issues with plutus-agent setup.
"""
...
_DHH = display_hermes_home()  # user-facing display path (e.g. ~/.plutus or ~/.plutus/profiles/coder)

# Load environment variables from ~/.plutus/.env so API key checks work
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.plutus/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `plutus`.
    ...
    print(color("│                 🩺 Plutus Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.plutus/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'plutus setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'plutus setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'plutus setup' to create one")
                issues.append("Run 'plutus setup' to create .env")
    
    # Check ~/.plutus/config.yaml (primary) or project cli-config.yaml (fallback)
            ...
            if provider and provider != "auto":
                if canonical_provider is None or (known_providers and canonical_provider not in known_providers):
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'plutus config set model.provider <valid_provider>'"
                    )
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.plutus/.env or run 'plutus setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{canonical_provider}'. "
                                f"Run 'plutus setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'plutus config set model.provider <name>'"
                            )
                except Exception:
                    pass
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'plutus setup' to migrate config")
                else:
                    issues.append("Run 'plutus doctor --fix' or 'plutus setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'plutus doctor --fix'")
        except Exception:
            pass
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'plutus doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (plutus bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "plutus"
        ...
        _cmd_link = _cmd_link_dir / "plutus"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(plutus not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/plutus → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/plutus points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/plutus → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/plutus — run 'plutus doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/plutus exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/plutus not found",
                    "(plutus command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/plutus → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/plutus symlink — run 'plutus doctor --fix'")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                issues.append(
                    ...
                    "Fix: run 'plutus config set model.provider <provider>' to switch providers, "
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'plutus setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: plutus memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: plutus memory setup")
                issues.append("No Honcho API key — run 'plutus memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run plutus memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: plutus memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: plutus memory setup")
        except Exception as _e:
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "plutus -p" in content:
                            _m = _re.search(r"plutus -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'plutus doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
