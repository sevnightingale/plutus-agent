...
Shows the status of all plutus-agent components.
"""
...
def show_status(args):
    """Show status of all plutus-agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ◆ plutus-agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: plutus model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: plutus model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: plutus model)"
    ...
    if not terminal_env:
        ...
        # (plutus status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    try:
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   plutus gateway")
    ...
    print(color("  Run 'plutus doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'plutus setup' to configure", Colors.DIM))
    print()
