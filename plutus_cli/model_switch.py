...
- ``plutus_cli.providers``        -- canonical provider identity + overlays
- ``plutus_cli.model_normalize``  -- per-provider name formatting
...
_HERMES_MODEL_WARNING = (
    ...
    "for use with plutus-agent. They lack the tool-calling capabilities "
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'plutus model' for available providers, or define it "
    ...
    # Mirrors the same logic in plutus_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from plutus_cli/models.py (OPENROUTER_MODELS,
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # (see plutus_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, plutus_cli/config.py) already consume that dict.
