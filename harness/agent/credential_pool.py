    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                from harness.agent.anthropic_adapter import refresh_anthropic_oauth_pure
                ...
                if entry.source == "claude_code":
                    try:
                        from harness.agent.anthropic_adapter import _write_claude_code_credentials
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `plutus auth remove <provider> <N>` is stable across all source types.
    try:
        from harness.cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        from harness.agent.anthropic_adapter import read_claude_code_credentials, read_hermes_oauth_credentials
        ...
        if state and not _is_suppressed(provider, "device_code"):
            ...
            # `plutus auth add nous --label <name>`).  Fall back to the
        ...
        try:
            from harness.cli.auth import resolve_qwen_runtime_credentials
        ...
        # Respect user suppression — `plutus auth remove openai-codex` marks
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    changed = False
    active_sources: Set[str] = set()
    # Honour user suppression — `plutus auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.plutus-agent/.env.
    ...
    try:
        from harness.cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
