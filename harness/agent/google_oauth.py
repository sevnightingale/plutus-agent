...
Storage (``~/.plutus-agent/auth/google_oauth.json``, chmod 0o600):
...
def _require_client_id() -> str:
    ...
    if not cid:
        raise GoogleOAuthError(
            ...
            "  2. Set HERMES_GEMINI_CLIENT_ID and HERMES_GEMINI_CLIENT_SECRET in ~/.plutus-agent/.env\n"
            "\n"
...
def get_valid_access_token(*, force_refresh: bool = False) -> str:
    ...
    if creds is None:
        raise GoogleOAuthError(
            "No Google OAuth credentials found. Run `plutus login --provider google-gemini-cli` first.",
