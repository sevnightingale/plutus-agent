...
Hermes seeds its credential pool from many places:
    ...
    env:<VAR>     — os.environ / ~/.plutus-agent/.env
    claude_code   — ~/.claude/.credentials.json
    hermes_pkce   — ~/.plutus-agent/.anthropic_oauth.json
    ...
    manual        — user ran `plutus auth add`
...
unify here is **removal**:
    ...
    ``plutus auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.plutus-agent/.env  → clear it
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.plutus-agent/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Hermes.",
            f"  The pool entry is now suppressed — Plutus will ignore "
            f"{env_var} until you run `plutus auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `plutus auth add anthropic` to re-enable if needed.",
    ])
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `plutus login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `plutus auth add nous` to re-engage, which is the documented re-add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    via ``plutus auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `plutus auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `plutus auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `plutus auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        ...
        description="~/.plutus-agent/.anthropic_oauth.json",
    ))
    register(RemovalStep(
