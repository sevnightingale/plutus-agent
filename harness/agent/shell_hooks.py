...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`plutus_cli.plugins.invoke_hook` and its aggregators.  Python
...
* First-use consent is gated by the allowlist under
  ``~/.plutus-agent/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``HERMES_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
  for registration to succeed without a prompt.
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``plutus_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``plutus_cli.config.load_config``
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`plutus_cli.plugins.get_pre_tool_call_block_message`.  This is
...
# ---------------------------------------------------------------------------
# Introspection (used by `plutus hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`plutus_cli.plugins.invoke_hook`
