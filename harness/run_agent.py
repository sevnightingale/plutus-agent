...
class AIAgent:
    ...
    def __init__(
            ...
            if api_key and base_url:
                ...
                if base_url_host_matches(effective_base, "openrouter.ai"):
                    client_kwargs["default_headers"] = {
                        "HTTP-Referer": "https://github.com/sevnightingale/plutus-agent",
                        "X-OpenRouter-Title": "Plutus Agent",
                ...
                if _routed_client is not None:
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        ...
                        raise RuntimeError(
                            ...
                            f"variable, or switch to a different provider with `plutus model`."
                        )
                    # No provider configured — reject with a clear message.
                    raise RuntimeError(
                        "No LLM provider configured. Run `plutus model` to "
                        "select a provider, or run `plutus setup` for first-time "
                        "configuration."
                    )
        ...
        # Session logs go into ~/.plutus-agent/sessions/ alongside gateway sessions
        ...
        # External memory-provider subsystem REMOVED. Built-in file-backed memory
        # (the `memory` tool + _memory_store above) is unaffected; the desk's
        # curated knowledge lives in the PLUTUS.md Lessons zone (rebuild R2).
        if not skip_memory:
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by Plutus.  Choose a model with at least "
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``plutus chat -q``. In that mode,
        ...
        try:
            ...
            if client is None or not aux_model:
                msg = (
                    ...
                    "Run `plutus setup` or set OPENROUTER_API_KEY."
                )
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        if not self.skip_context_files:
            # Context-file discovery (AGENTS.md / CLAUDE.md / .cursorrules),
            # driven by TERMINAL_CWD (bridged from terminal.cwd). When it's
            # unset or a placeholder ('.', 'auto', 'cwd'), fall back to
            # HERMES_HOME — the data dir, which holds no dev context files —
            # NOT os.getcwd(). os.getcwd() is the install/repo dir, so it would
            # inject the harness's own AGENTS.md ("Plutus - Development
            # Guide") into the identity slot and make the agent introduce
            # itself as Hermes (real bug, 2026-06-03), besides wasting ~10k
            # tokens on dev docs of no use to the trading agent.
    ...
    def _compress_context(self, messages: list, system_message: str, *, approx_tokens: int = None, task_id: str = "default", focus_topic: str = None, layer_hint: str = "agent_mid_conversation") -> tuple:
        ...
        Args:
            ...
            layer_hint: V2 compaction visibility — which layer triggered this
                compression. "agent_mid_conversation" (default) when the agent
                loop's compressor fires; "gateway_pre_compress" when the
                gateway's session-hygiene pre-compress runs before a turn.
                Recorded into the compaction event for plutus-main's
                visibility into context lifecycle.

        Returns:
            (compressed_messages, new_system_prompt) tuple
        """
        _pre_msg_count = len(messages)
        _pre_session_id = self.session_id  # capture BEFORE rotation below
        logger.info(
        ...
        # V2 compaction visibility — record an event for plutus-main to query
        ...
        try:
            ...
            if _evt and _evt.fn:
                _evt.fn(
                    layer=layer_hint,
                    pre_token_count=int(approx_tokens or 0),
                    post_token_count=int(_compressed_est or 0),
                    pre_message_count=_pre_msg_count,
                    post_message_count=len(compressed),
                    session_id_before=_pre_session_id,
                    session_id_after=self.session_id,
                    model=self.model,
                    focus_topic=focus_topic,
                )
        except Exception as _evt_err:
            logger.debug("compaction event record failed (non-fatal): %s", _evt_err)
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if (
                        ...
                        print(f"{self.log_prefix}     • Re-authenticate: plutus login --provider nous")
                    ...
                    if (
                        ...
                        print(f"{self.log_prefix}     • Legacy cleanup: plutus config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: plutus config set ANTHROPIC_API_KEY \"\"")
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `plutus auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: plutus setup", force=True)
