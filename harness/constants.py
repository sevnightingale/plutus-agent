...
# ``~/.plutus-agent`` to stay isolated from upstream Hermes's ``~/.hermes``.
# Anything that needs to derive the dir name (systemd unit generation, path
# remapping for sudo installs, etc.) should import this rather than hardcode.
HERMES_HOME_DIR_NAME = ".plutus-agent"


def get_hermes_home() -> Path:
    """Return the Hermes home directory (default: ~/.plutus-agent).

    Reads HERMES_HOME env var, falls back to ``~/`` + HERMES_HOME_DIR_NAME.
    This is the single source of truth — all other copies should import this.

    Note: plutus-agent uses ``~/.plutus-agent/`` as its default home dir
...
def get_default_hermes_root() -> Path:
    ...
    In standard plutus-agent deployments this is ``~/.plutus-agent``.

    In Docker or custom deployments where ``HERMES_HOME`` points outside
    ``~/.plutus-agent`` (e.g. ``/opt/data``), returns ``HERMES_HOME``
    directly — that IS the root.

    In profile mode where ``HERMES_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.plutus-agent/profiles/strategy-a``)
    and Docker (``/opt/data/profiles/strategy-a``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = Path.home() / ".plutus-agent"
...
def display_hermes_home() -> str:
    ...
    Uses ``~/`` shorthand for readability::
        ...
        default:  ``~/.hermes``
        profile:  ``~/.plutus-agent/profiles/coder``
...
def get_config_path() -> Path:
    ...
    in 7+ files (skill_utils.py, plutus_logging.py, plutus_time.py, etc.).
    """
