...
Usage:
    from harness.tools.browser_tool import browser_navigate, browser_snapshot, browser_click
...
try:
    from harness.tools.url_safety import is_safe_url as _is_safe_url
except Exception:
    _is_safe_url = lambda url: False  # noqa: E731 — fail-closed: block all if safety module unavailable
from harness.tools.browser_providers.base import CloudBrowserProvider
from harness.tools.browser_providers.browserbase import BrowserbaseProvider
from harness.tools.browser_providers.browser_use import BrowserUseProvider
from harness.tools.browser_providers.firecrawl import FirecrawlProvider
from harness.tools.tool_backend_helpers import normalize_browser_cloud_provider

# Camofox local anti-detection browser backend (optional).
# When CAMOFOX_URL is set, all browser operations route through the
# camofox REST API instead of the agent-browser CLI.
try:
    from harness.tools.browser_camofox import is_camofox_mode as _is_camofox_mode
except ImportError:
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from harness.tools.browser_camofox import camofox_navigate
...
def browser_snapshot(
    ...
    """
    if _is_camofox_mode():
        from harness.tools.browser_camofox import camofox_snapshot
...
def browser_click(ref: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from harness.tools.browser_camofox import camofox_click
...
def browser_type(ref: str, text: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from harness.tools.browser_camofox import camofox_type
...
def browser_scroll(direction: str, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from harness.tools.browser_camofox import camofox_scroll
...
def browser_back(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from harness.tools.browser_camofox import camofox_back
...
def browser_press(key: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from harness.tools.browser_camofox import camofox_press
...
def browser_console(clear: bool = False, expression: Optional[str] = None, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from harness.tools.browser_camofox import camofox_console
...
def _camofox_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    from harness.tools.browser_camofox import _ensure_tab, _post
    try:
...
def browser_get_images(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from harness.tools.browser_camofox import camofox_get_images
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from harness.tools.browser_camofox import camofox_vision
...
def cleanup_browser(task_id: Optional[str] = None) -> None:
    ...
    if _is_camofox_mode():
        try:
            from harness.tools.browser_camofox import camofox_close, camofox_soft_cleanup
