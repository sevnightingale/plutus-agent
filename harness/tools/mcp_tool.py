...
Configuration is read from ~/.plutus-agent/config.yaml under the ``mcp_servers`` key.
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"file under ~/.plutus-agent/mcp-tokens/ and restart). Do NOT retry "
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``plutus tools`` interactive configuration — connects to each
