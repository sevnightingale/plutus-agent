"""
Cron job management tools for Plutus.
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via plutus model.
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    # Only relative paths within ~/.plutus-agent/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.plutus-agent/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.plutus-agent/scripts/ and use just the filename."
        )
