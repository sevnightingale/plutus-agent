...
Configuration is loaded from ~/.plutus-agent/config.yaml under the 'tts:' key.
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.plutus-agent/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.plutus-agent/config.yaml.
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.plutus-agent/config.yaml (tts: section).
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run plutus setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'plutus setup tts' and choose KittenTTS, or install manually: "
