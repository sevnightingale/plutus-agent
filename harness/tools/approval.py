...
DANGEROUS_PATTERNS = [
    ...
    (r'\b(?:hermes|plutus)\s+gateway\s+(stop|restart)\b', "stop/restart plutus gateway (kills running agents — use request_desk_restart)"),
    (r'\b(?:hermes|plutus)\s+update\b', "plutus update (restarts gateway, kills running agents)"),
...
def _is_unattended_origin_turn() -> bool:
    """True when the current turn was system-injected with no operator present.

    Two paths can mark a turn as unattended:

    1. **Unified-session (preferred):** ``synthetic_kind`` ContextVar set by
       the gateway when a turn arrived via ``deliver_synthetic_message``
       (cron tick or watcher wake event in the unified-session model). This
       is per-turn and asyncio-task-local — concurrency-safe.
    2. **Legacy fresh-session fallback:** ``HERMES_CRON_SESSION`` env var
       set by ``cron.scheduler._legacy_run_job`` when it spawns its own
       AIAgent (used when no gateway is available — standalone daemon —
       or for jobs without an origin). Process-global, so we explicitly
       gate it on "not currently in a gateway session" to avoid leaking
       into gateway turns when both run in the same process.

    Both must apply cron-mode policy: nobody is present to approve dangerous
    commands. Returning True here makes the dangerous-command guard refuse
    rather than block waiting for an approval that will never arrive.
    """
    ...
    # 2. Legacy env var, but ONLY when we're outside both an interactive
    # CLI session AND a gateway session. The legacy HERMES_CRON_SESSION
    # is a process-global from cron.scheduler._legacy_run_job; without
    # this gate, a stale value (set earlier in the same process or in a
    # leaky test) would mis-block legitimate operator-driven dangerous
    # commands. Inside a gateway/CLI session, only the synthetic_kind
    # ContextVar is authoritative.
    if (
        os.getenv("HERMES_CRON_SESSION")
        and not os.getenv("HERMES_GATEWAY_SESSION")
        and not os.getenv("HERMES_INTERACTIVE")
    ):
        return True
...
def check_dangerous_command(command: str, env_type: str,
    ...
    # Unified-session: cron ticks and wake events arrive THROUGH the gateway,
    # so the legacy "outside CLI/gateway" branch below would skip the cron
    # check. Apply cron-mode policy first based on synthetic origin —
    # operator is not present even though the turn looks like a gateway turn.
    if _is_unattended_origin_turn():
        if _get_cron_approval_mode() == "deny":
            return {
                "approved": False,
                "message": (
                    f"BLOCKED: Command flagged as dangerous ({description}) "
                    "but this turn was system-triggered (cron tick or watcher "
                    "wake) with no operator present to approve it. "
                    "Find an alternative approach that avoids this command. "
                    "To allow dangerous commands in unattended turns, set "
...
def check_all_command_guards(command: str, env_type: str,
    ...
    # Unified-session: cron ticks and wake events arrive THROUGH the gateway.
    # Apply cron-mode policy first based on synthetic origin — operator is
    # not present even though the turn looks like a gateway turn.
    if _is_unattended_origin_turn():
        if _get_cron_approval_mode() == "deny":
