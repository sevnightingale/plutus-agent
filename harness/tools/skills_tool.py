...
# All skills live in ~/.plutus-agent/skills/ (seeded from bundled skills/ on install).
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.plutus-agent/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.plutus-agent/skills/ and external dirs.
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.plutus-agent/skills/): {skill_md}")
