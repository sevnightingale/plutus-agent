...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.plutus-agent/.env manually."
)
...
class MessageEvent:
    ...
    # Origin marker for system-injected events (cron tick, watcher wake,
    # manual inject). None for real platform inbound. Examples:
    #   "cron:plutus-heartbeat" — fired from cron scheduler
    #   "wake:hl_position_status_change" — fired from watcher daemon
    #   "manual" — operator-tool inject for testing
    # When set, the gateway: (a) skips user-authorization checks (in
    # combination with internal=True), (b) routes through the busy handler
    # in queue-without-interrupt mode, (c) suppresses outbound chat send
    # when the agent's response is empty/(empty) (a quiet tick is silence).
    synthetic_kind: Optional[str] = None

    # Optional future used by deliver_synthetic_message to await the agent's
    # response from a synthetic-injection caller (cron, watcher). When set,
    # the background processing task resolves this future with the response
    # text (or None when displaced/interrupted) so the caller can await a
    # return value instead of polling. Real platform inbound leaves this
    # None — those callers do not block waiting for the response.
    response_future: Optional[Any] = None

    # Outbound delivery override — when set, the post-turn flow sends the
    # agent's response (typing indicator, text, media, TTS, all of it) to
    # ``reply_to_chat_id`` instead of ``source.chat_id``.  The session_key
    # and identity still derive from ``source``, so the event lands in the
    # source-session for processing but the reply emerges in the override
    # chat.  Used by the Manor-mention redirect: a Manor mention of the
    # bot is routed into the operator's unified DM session (source.chat_id
    # = DM id) but Plutus's reply goes back to Manor (reply_to_chat_id =
    # group id).  ``reply_to_thread_id`` allows the same override for
    # forum-topic targets.  Both None on plain inbound — no behavioural
...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            # If a prior pending message had a synthetic-injection caller
            # waiting on it, resolve its future so the caller doesn't hang.
            # The displaced event won't be processed; treat as "no response."
            _existing = self._pending_messages.get(session_key)
            if _existing is not None:
                _existing_fut = getattr(_existing, "response_future", None)
                if _existing_fut is not None and not _existing_fut.done():
                    try:
                        _existing_fut.set_result(None)
                    except Exception:
                        pass
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        # Outbound delivery target — defaults to the source chat for normal
        # inbound, but overrides to ``reply_to_chat_id`` when the event was
        # built by the Manor-mention redirect (or any other future redirect
        # caller).  The session_key still derives from ``source``, so the
        # event lands in the source-session for processing; only the
        # outbound delivery is redirected.
        _target_chat_id = event.reply_to_chat_id or event.source.chat_id
        _target_thread_id = event.reply_to_thread_id or event.source.thread_id

        # Start continuous typing indicator (refreshes every 2 seconds)
        _thread_metadata = {"thread_id": _target_thread_id} if _target_thread_id else None
        ...
        try:
            ...
            # Resolve the synthetic-injection caller's future (if any) with the
            # agent's response.  This is the unified-session bridge that lets
            # cron/scheduler.deliver_synthetic_message capture Plutus's
            # response into the cron output file.  Done BEFORE the platform
            # send so the scheduler can record the text even if the platform
            # send fails for any reason.
            _resp_fut = getattr(event, "response_future", None)
            if _resp_fut is not None and not _resp_fut.done():
                try:
                    _resp_fut.set_result(response)
                except Exception:
                    pass
            if response:
                ...
                # Checks the target chat (where the response lands) so a Manor-mention
                # redirected to the DM session still honours Manor's /voice setting.
                _tts_path = None
                ...
                if text_content:
                    logger.info("[%s] Sending response (%d chars) to %s", self.name, len(text_content), _target_chat_id)
            ...
            # Last-resort: if a synthetic-injection caller is still waiting on
            # this event's response_future, resolve it with None so the
            # caller doesn't hang past its timeout.  The happy path resolves
            # the future earlier with the actual response text.
            _resp_fut = getattr(event, "response_future", None)
            if _resp_fut is not None and not _resp_fut.done():
                try:
                    _resp_fut.set_result(None)
                except Exception:
                    pass
