...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Unified-session redirect bindings (Manor-mention path).  Maps a
        # group chat_id (str) to a redirect config dict:
        #   {"target_dm_chat_id": str,
        #    "target_dm_user_id": str,
        #    "marker_prefix": str}
        # Off by default — populated only when ``telegram.unified_session_redirect``
        # is set in config.yaml.  See gateway/platforms/base.py
        # MessageEvent.reply_to_chat_id and base.py
        # _process_message_background for the outbound side of the redirect.
        self._unified_redirect_bindings: Dict[str, Dict[str, str]] = self._load_unified_redirect_bindings()
        if self._unified_redirect_bindings:
            logger.info(
                "[%s] Loaded %d unified-session redirect binding(s): %s",
                self.name,
                len(self._unified_redirect_bindings),
                list(self._unified_redirect_bindings.keys()),
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``plutus update --gateway``
    ...
    # ── Unified-session redirect (Manor-mention path) ────────────────────

    def _load_unified_redirect_bindings(self) -> Dict[str, Dict[str, str]]:
        """Parse ``telegram.unified_session_redirect`` from config into a
        lookup dict keyed by source_group_id.

        Returns an empty dict when the config key is missing or empty —
        in that case the redirect path is dormant and inbound messages
        flow normally.  Invalid entries are dropped with a warning so a
        partial misconfiguration doesn't disable the working bindings.
        """
        raw = self.config.extra.get("unified_session_redirect", [])
        if not raw:
            return {}
        if not isinstance(raw, list):
            logger.warning(
                "[%s] telegram.unified_session_redirect must be a list, got %s — ignoring",
        ...
        for idx, entry in enumerate(raw):
            if not isinstance(entry, dict):
                logger.warning(
                    "[%s] unified_session_redirect[%d] must be a dict, got %s — skipping",
                    self.name, idx, type(entry).__name__,
                )
                continue
            src = entry.get("source_group_id")
            dst = entry.get("target_dm_chat_id")
            usr = entry.get("target_dm_user_id") or dst
            marker = entry.get("marker_prefix") or "[MANOR MENTION"
            if not src or not dst:
                logger.warning(
                    "[%s] unified_session_redirect[%d] missing source_group_id or target_dm_chat_id — skipping",
                    self.name, idx,
                )
                continue
            out[str(src)] = {
                "target_dm_chat_id": str(dst),
                "target_dm_user_id": str(usr),
                "marker_prefix": str(marker),
            }
        return out

    def _maybe_redirect_event(self, event: MessageEvent) -> MessageEvent:
        """Return a redirected MessageEvent if this is a Manor-mention
        candidate, otherwise return ``event`` unchanged.

        Trigger: the source chat_id is a configured redirect-source group AND
        the inbound Telegram message either @-mentions the bot or replies
        to a bot message.  In that case we build a new MessageEvent whose
        ``source.chat_id`` points to the configured unified-session DM
        (so session_key resolves to the DM session) and whose
        ``reply_to_chat_id`` carries the original group chat_id (so the
        post-turn delivery sends the reply back to the group).  The
        original ``raw_message``, ``message_id``, attachments, and
        ``reply_to_message_id`` are preserved so Telegram threading and
        media handling work normally.

        Callers should pass the returned event to ``super().handle_message``
        regardless of whether redirect fired — the no-op case returns the
        input event unmodified.
        """
        if not self._unified_redirect_bindings:
            return event
        ...
        if not src_chat_id or src_chat_id not in self._unified_redirect_bindings:
            return event
        # Only redirect when the inbound is in a group/supergroup AND addressed
        # to the bot.  Non-mention chatter in the same group flows normally.
        if not self._is_group_chat(raw):
            return event
        if not (self._message_mentions_bot(raw) or self._is_reply_to_bot(raw)):
            return event

        binding = self._unified_redirect_bindings[src_chat_id]
        target_chat_id = binding["target_dm_chat_id"]
        target_user_id = binding["target_dm_user_id"]
        marker_prefix = binding["marker_prefix"]

        # Build the wrapped text so plutus-main can distinguish a Manor
        # mention from a cron synthetic tick or a DM turn.  Format:
        # "[MANOR MENTION — from @<username> at <chat_title> — <iso ts>]\n<text>"
        sender = getattr(source, "user_name", None) or "unknown"
        try:
            chat_title = getattr(raw.chat, "title", None) or "Manor"
        except Exception:
            chat_title = "Manor"
        try:
            ...
            ts_iso = ""
        marker_line = f"{marker_prefix} — from {sender} at {chat_title}"
        if ts_iso:
            marker_line += f" at {ts_iso}"
        marker_line += "]"
        wrapped_text = f"{marker_line}\n{event.text}" if event.text else marker_line

        # New SessionSource whose chat_id is the DM (so session_key
        # resolves to the unified session).  user_id is the operator's
        # so any per-user-attribution downstream (memory, authz) sees
        # the operator's identity, not the original Manor sender.
        new_source = SessionSource(
            platform=source.platform,
            chat_id=target_chat_id,
            chat_type="dm",
            user_id=target_user_id,
            user_name=getattr(source, "user_name", None),  # preserve original sender's display name
            thread_id=None,
        ...
        # Build the redirected event.  Preserve raw_message + media so
        # downstream handlers continue to work; only the routing fields
        # change.  ``reply_to_chat_id`` / ``reply_to_thread_id`` carry the
        # original group address so the post-turn delivery sends the
        # response back to the group instead of the DM.
        redirected = MessageEvent(
            text=wrapped_text,
            ...
            internal=True,
            synthetic_kind=f"manor_mention:{src_chat_id}",
            reply_to_chat_id=src_chat_id,
            reply_to_thread_id=getattr(source, "thread_id", None),
            timestamp=event.timestamp,
        )
        logger.info(
            "[%s] Manor-mention redirect: group=%s → DM=%s (sender=%s, kind=%s)",
            self.name,
            src_chat_id,
            target_chat_id,
            sender,
            redirected.synthetic_kind,
        )
        return redirected

    async def handle_message(self, event: MessageEvent) -> None:
        """Telegram override: apply Manor-mention redirect before base routing.

        When the inbound event qualifies for unified-session redirect
        (Manor mention or reply-to-bot in a configured group), rewrite
        it so its ``source.chat_id`` points to the configured DM and
        its ``reply_to_chat_id`` points back to the group.  Then delegate
        to the base implementation for session-keyed routing, busy-handler
        queueing, command dispatch, and background processing.
        """
        event = self._maybe_redirect_event(event)
