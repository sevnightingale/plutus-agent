...
# Synthetic-origin marker for the current turn. Set by the gateway when a
# turn is system-injected (cron tick or watcher wake) via
# deliver_synthetic_message; unset for normal operator-driven turns.
# Used by tools/approval.py to apply cron-mode policy in unified-session mode
# (where HERMES_CRON_SESSION env var is no longer set per-tick).
_SYNTHETIC_KIND: ContextVar = ContextVar("HERMES_SYNTHETIC_KIND", default=_UNSET)
...
_VAR_MAP = {
    ...
    "HERMES_SYNTHETIC_KIND": _SYNTHETIC_KIND,
}
...
def set_synthetic_kind(kind: str | None) -> object:
    """Mark the current turn as system-injected (cron/wake) for tools that
    introspect origin (notably tools/approval.py for cron-mode policy).

    Returns a Token usable with reset_synthetic_kind() to restore the prior
    value (typically called in a finally block at the end of the turn).
    """
    return _SYNTHETIC_KIND.set(kind or "")


def reset_synthetic_kind(token: object) -> None:
    """Restore the synthetic_kind ContextVar to its prior value."""
    try:
        _SYNTHETIC_KIND.reset(token)
    except Exception:
        # Best-effort restore; if the token is from a different context
        # (rare) just clear to empty to avoid leaking the marker.
        _SYNTHETIC_KIND.set("")


def get_synthetic_kind() -> str:
    """Read the current turn's synthetic_kind marker.

    Returns:
        Empty string if not set or explicitly cleared. Strings like
        "cron:plutus-heartbeat" or "wake:hl_position_status_change" when
        the turn was injected by the cron scheduler or watcher tailer.
    """
    value = _SYNTHETIC_KIND.get()
    if value is _UNSET:
        return ""
    return value or ""
