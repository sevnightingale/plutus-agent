"""
Configuration management for plutus-agent.

Config files are stored in ~/.plutus-agent/ for easy access:
- ~/.plutus-agent/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.plutus-agent/.env         - API keys and secrets

This module provides:
- plutus config          - Show current configuration
- plutus config edit     - Open config in editor
- plutus config set      - Set a specific value
- plutus config wizard   - Re-run setup wizard
"""
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "plutus update"
...
def _secure_dir(path):
    ...
    if is_managed():
        return
    try:
        mode_str = (os.environ.get("PLUTUS_HOME_MODE") or os.environ.get("HERMES_HOME_MODE", "")).strip()
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["plutus-agent-cli"],  # plutus-agent fork default. Set to ["hermes-cli"] for upstream Hermes behavior.
    "agent": {
        "max_turns": 90,
    ...
    "display": {
        ...
        "skin": "plutus-agent",  # plutus-agent fork default; set to "default" for upstream Hermes look
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.plutus-agent/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "notifications": {
        ...
        # close_position / dgclaw_trade_*) get a one-line summary posted
        # via the gateway's Telegram bot. Empty string disables.
        "trade_chat_id": "",
        # Telegram chat id for HALT / drawdown / tilt-detection alerts —
        # things the operator needs to know about even when not actively
        # watching the trade-notify channel. Falls back to trade_chat_id
        # when empty.
        "alert_chat_id": "",
    },
    ...
    # always goes to ~/.plutus-agent/skills/.
    "skills": {
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "◆ *plutus-agent*" header.
    ...
    "telegram": {
        ...
        # Manor-mention redirect bindings.  When a group message in
        # ``source_group_id`` mentions the bot (or replies to a bot
        # message), the inbound event is rewritten so its session_key
        # resolves to ``target_dm_chat_id`` (a DM) instead of the
        # per-user group session.  The agent's reply still emerges in
        # the source group via the ``reply_to_chat_id`` override on
        # MessageEvent.  Net effect: a single unified session sees both
        # cron synthetic-injections (which already target the DM) and
        # group mentions of the bot, so the agent's reasoning is
        # continuous across channels.
        #
        # Schema (list of bindings):
        #   - source_group_id:   "-1003793147758"  # supergroup id
        #     target_dm_chat_id: "1054536871"      # operator's DM chat id
        #     target_dm_user_id: "1054536871"      # operator's user id (for SessionSource.user_id)
        #     marker_prefix:     "[MANOR MENTION"  # prepended to redirected text
        #
        # Off by default (empty list).  Multiple bindings can target
        # different DM sessions for different groups.
        "unified_session_redirect": [],
    },
    ...
    # stored approval from ~/.plutus-agent/shell-hooks-allowlist.json.
    ...
    # Logging — controls file logging to ~/.plutus-agent/logs/.
    ...
    # Session storage — controls automatic cleanup of ~/.plutus-agent/state.db.
    ...
    "sessions": {
        ...
        # default of ``plutus sessions prune``.
        "retention_days": 90,
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'plutus setup' to create a valid config")]
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'plutus doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: plutus config set <key> <value>")
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.plutus-agent/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.plutus-agent/config.yaml."""
    ensure_hermes_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — plutus auth) — OpenAI Codex
#   nous         (OAuth — plutus auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.plutus-agent/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.plutus-agent/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.plutus-agent/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.plutus-agent/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.plutus-agent/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.plutus-agent/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Plutus — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known vars that are no longer in .env
    for key in known_keys:
        if key not in env_vars and key in os.environ:
            del os.environ[key]
            count += 1
    # Re-bridge PLUTUS_* → HERMES_* for vars that arrived via .env.
    from harness.constants import alias_plutus_env
    alias_plutus_env()
    return count


def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.plutus-agent/.env or environment."""
...
def show_config():
    ...
    print(color("│              ◆ plutus-agent Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  plutus config edit     # Edit config file", Colors.DIM))
    print(color("  plutus config set <key> <value>", Colors.DIM))
    print(color("  plutus setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: plutus config set <key> <value>")
            print()
            print("Examples:")
            print("  plutus config set model anthropic/claude-sonnet-4")
            print("  plutus config set terminal.backend docker")
            print("  plutus config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'plutus config migrate' to add them")
        ...
        print("  plutus config           Show current configuration")
        print("  plutus config edit      Open config in editor")
        print("  plutus config set <key> <value>   Set a config value")
        print("  plutus config check     Check for missing/outdated config")
        print("  plutus config migrate   Update config with new options")
        print("  plutus config path      Show config file path")
        print("  plutus config env-path  Show .env file path")
        sys.exit(1)
