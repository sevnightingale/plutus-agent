...
Add, remove, or reorder entries here — both `plutus setup` and
...
# this list:  plutus model, /model, /provider, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `plutus model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `plutus model` TUI


CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("opencode-go",    "OpenCode Go",              "OpenCode Go (open models, $10/month subscription — recommended for plutus-agent)"),
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``plutus model``, ``/model``, etc.).
    """
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``plutus_cli.auth.resolve_provider()`` to resolve it to a concrete
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Plutus will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
