...
Consent records live under ``~/.plutus-agent/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.plutus-agent/config.yaml``
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.plutus-agent/config.yaml.")
