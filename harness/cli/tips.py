...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or plutus -c.",
    ...
    # --- CLI Flags ---
    "plutus -c resumes your most recent CLI session. plutus -c \"project name\" resumes by title.",
    "plutus -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "plutus -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "plutus chat -t web,terminal enables only specific toolsets for a focused session.",
    "plutus chat -s github-pr-workflow preloads a skill at launch.",
    "plutus chat -q \"query\" runs a single non-interactive query and exits.",
    "plutus chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "plutus chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hermes --yolo bypasses all dangerous command approval prompts for the entire session.",
    "plutus chat --source telegram tags the session for filtering in plutus sessions list.",
    "hermes -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "plutus doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "hermes dump outputs a compact setup summary — great for bug reports.",
    "plutus config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "plutus config edit opens config.yaml in your default editor.",
    "plutus config check scans for missing or stale configuration options.",
    "plutus sessions browse opens an interactive session picker with search.",
    "plutus sessions stats shows session counts by platform and database size.",
    "plutus sessions prune --older-than 30 cleans up old sessions.",
    "plutus skills search react --source skills-sh searches the skills.sh public directory.",
    "plutus skills check scans installed hub skills for upstream updates.",
    "plutus skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "plutus skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    ...
    "plutus auth add lets you add multiple API keys for credential pool rotation.",
    ...
    "plutus update syncs new bundled skills to ALL profiles automatically.",
    "plutus gateway install sets up Hermes as a system service (systemd/launchd).",
    "plutus memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "plutus webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as plutus -w).",
    ...
    "plutus sessions export backup.jsonl exports all sessions for backup or analysis.",
    ...
    "plutus skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: plutus cron add --skill polymarket \"Check for new markets\".",
    ...
    "plutus gateway install sets it up as a system service that starts on boot.",
    ...
    "PLUTUS.md at ~/.plutus-agent/PLUTUS.md is the agent's identity — doctrine, live state, and lessons in one file.",
    ...
    "MCP servers auto-generate toolsets at runtime — plutus tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.plutus-agent/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "plutus chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "plutus acp runs Hermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "plutus login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "plutus status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.plutus-agent/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.plutus-agent/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "PLUTUS.md completely replaces the agent's default identity — its Doctrine zone is yours to shape.",
    "PLUTUS.md is created at first boot (setup or gateway start). Edit the Doctrine zone; Live State and Lessons belong to the desk.",
    ...
    "The skills audit log at ~/.plutus-agent/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to ~/.plutus-agent/interrupt_debug.log with timestamps.",
    ...
    "hermes profile create backup --clone-all copies everything (config, keys, PLUTUS.md, memories, skills, sessions).",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via plutus tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.plutus-agent/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.plutus-agent/skills/.hub/quarantine/ holds skills pending security review.",
]
