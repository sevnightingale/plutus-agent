"""
Interactive setup wizard for plutus-agent.
...
Config files are stored in ~/.plutus/ for easy access.
"""
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("◆ Plutus Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Plutus using environment variables or config commands:")
    print_info("  plutus config set model.provider custom")
    print_info("  plutus config set model.base_url http://localhost:8080/v1")
    print_info("  plutus config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'plutus setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'plutus setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'plutus setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'plutus setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'plutus setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'plutus setup terminal'"))
    ...
    print()
    print(f"   {color('plutus setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('plutus setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('plutus setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('plutus setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('plutus setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('plutus config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('plutus config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('plutus config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('plutus', Colors.GREEN)}              Start chatting")
    print(f"   {color('plutus gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('plutus doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `plutus tools` and `plutus setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``plutus model``)
    ...
    provider added to ``plutus model`` is automatically available here.
    ...
    try:
        ...
        print_info("You can try again later with: plutus model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "Plutus can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Plutus can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'plutus setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.plutus/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'plutus setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Plutus runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Plutus via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `plutus setup agent` later to customize.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Plutus delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://github.com/sevnightingale/plutus-agent/user-guide/messaging/slack/")
    print()
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Plutus delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Plutus delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'plutus whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'plutus whatsapp' to choose your mode (separate bot number")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Plutus to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://github.com/sevnightingale/plutus-agent/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://github.com/sevnightingale/plutus-agent/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  plutus config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Plutus from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'plutus setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     plutus config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: plutus gateway install")
            else:
                print_info("  You can install later: plutus gateway install")
                if supports_systemd:
            ...
            if is_container():
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   plutus gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `plutus setup tools` and `plutus tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``plutus_cli.auth`` — lists every supported
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from plutus_cli.config
...
# Config values may have different semantics between OpenClaw and Plutus.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Plutus to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Plutus at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Plutus at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Plutus at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Plutus at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Plutus equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Plutus config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Plutus.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Plutus's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Plutus can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: plutus claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: plutus claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Plutus configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Plutus config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Plutus (use plutus claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      plutus setup           — full or quick (auto-detected)
      plutus setup model     — just model/provider
      plutus setup tts       — just text-to-speech
      plutus setup terminal  — just terminal backend
      plutus setup gateway   — just messaging platforms
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ◆ Plutus Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ◆ plutus-agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your plutus-agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Plutus configured.")
        print()
    ...
    print()
    print_info("You can edit these files directly or use 'plutus config edit'")
...
def _run_first_time_setup(config: dict, hermes_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'plutus setup gateway'",
        ],
        0,
    )
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'plutus setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Plutus to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'plutus setup gateway'.")
