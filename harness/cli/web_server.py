"""
plutus-agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m plutus_cli.main web          # Start on http://127.0.0.1:9119
    python -m plutus_cli.main web --port 8080
"""
...
app = FastAPI(title="plutus-agent", version=__version__)
...
# streamed to a per-action log file under ``~/.plutus-agent/logs/<action>.log`` so
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    Uses the running interpreter's ``plutus_cli.main`` module so the action
...
async def restart_gateway():
    """Kick off a ``plutus gateway restart`` in the background."""
    try:
...
async def update_hermes():
    """Kick off ``plutus update`` in the background."""
    try:
        proc = _spawn_hermes_action(["update"], "hermes-update")
    except Exception as exc:
        _log.exception("Failed to spawn plutus update")
...
# the canonical ``plutus auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.plutus-agent/.anthropic_oauth.json`` — Hermes-managed PKCE flow
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "plutus auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "plutus auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "plutus auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "plutus auth add qwen-oauth",
...
#          → persists to ~/.plutus-agent/.anthropic_oauth.json AND credential pool
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    the system in the same state as ``plutus auth add anthropic``.
    """
    from harness.agent.anthropic_adapter import _HERMES_OAUTH_FILE
    payload = {
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",   "label": "Plutus Teal",  "description": "Classic dark teal — the canonical plutus-agent look"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.plutus-agent/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.plutus-agent/dashboard-themes/*.yaml for user-created themes.
...
async def get_dashboard_themes():
    ...
    from `~/.plutus-agent/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as plutus_cli.plugins):
    1. User plugins:    ~/.plutus-agent/plugins/<name>/dashboard/manifest.json
...
def start_server(
    ...
    print(f"  Plutus Web UI → http://{host}:{port}")
