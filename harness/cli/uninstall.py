"""
plutus-agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.plutus-agent/ (configs, sessions, logs)
"""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# plutus-agent" comment and following line
                if '# plutus-agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains plutus_cli reference)
                content = wrapper.read_text()
                if 'plutus_cli' in content or 'hermes-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``plutus gateway run`` processes
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.plutus-agent/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.plutus-agent/ for future reinstall
    """
    ...
    print(color("│            ◆ plutus-agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.plutus-agent/, just remove the hermes-agent subdir
    ...
    # 5. Optionally remove ~/.plutus-agent/ data directory (and named profiles)
    if full_uninstall:
    ...
    print()
    print("Thank you for using plutus-agent! ◆")
    print()
