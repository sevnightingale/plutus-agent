"""
Multi-provider authentication system for plutus-agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.plutus-agent/auth.json with cross-process file locking.
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `plutus model` to re-authenticate."
...
# =============================================================================
# Auth Store — persistence layer for ~/.plutus-agent/auth.json
# =============================================================================
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `plutus logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'plutus doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'plutus model' for available providers, or run 'plutus doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'plutus model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.plutus-agent/.env.",
...
#
# Tokens live in ~/.plutus-agent/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `plutus auth list` / `plutus status`."""
    try:
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.plutus-agent/auth.json (not ~/.codex/)
#
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Hermes auth store (~/.plutus-agent/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `plutus auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `plutus auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `plutus auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `plutus auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Hermes auth store (~/.plutus-agent/auth.json)."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `plutus auth` to re-authenticate."
            )
            relogin_required = True
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``plutus auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``plutus auth add nous --label <name>``).  It gets embedded in the
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `plutus status` output.

    Checks the credential pool first (where the dashboard device-code flow
    and ``plutus auth`` store credentials), then falls back to the legacy
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `plutus auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `plutus auth` and
...
def login_command(args) -> None:
    """Deprecated: use 'plutus model' or 'plutus setup' instead."""
    print("The 'plutus login' command has been removed.")
    print("Use 'plutus auth' to manage credentials,")
    print("'plutus model' to select a provider, or 'plutus setup' for full setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.plutus-agent/auth.json."""
    ...
    *,
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `plutus model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `plutus model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            ...
            print("Run `plutus model` or configure an API key to use Hermes.")
    else:
