...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with hermes-agent;
   ...
   own discovery paths)
2. **User plugins**   – ``~/.plutus-agent/plugins/<name>/``
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``plutus honcho ...``).
    ...
    def register_skill(
        ...
        ``~/.plutus-agent/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to plutus_cli/. Two layouts are
        ...
        # 2. User plugins (~/.plutus-agent/plugins/)
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``plutus tools`` TUI so plugin-provided toolsets appear
