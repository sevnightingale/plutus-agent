"""Slash command definitions and autocomplete for the plutus-agent CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("update", "Update plutus-agent to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`plutus_cli.plugins.PluginContext.register_command`. They behave
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``plutus skills
