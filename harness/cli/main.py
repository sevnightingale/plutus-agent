#!/usr/bin/env python3
"""
plutus-agent CLI - Main entry point.

Usage:
    hermes                     # Interactive chat (default)
    plutus chat                # Interactive chat
    plutus gateway             # Run gateway in foreground
    plutus gateway start       # Start gateway as service
    plutus gateway stop        # Stop gateway service
    plutus gateway status      # Show gateway status
    plutus gateway install     # Install gateway service
    plutus gateway uninstall   # Uninstall gateway service
    plutus setup               # Interactive setup wizard
    plutus logout              # Clear stored authentication
    plutus status              # Show status of all components
    ...
    plutus acp                 Run as an ACP server for editor integration
    plutus sessions browse     Interactive session picker with search
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (plutus tools, plutus setup, plutus model) use
...
# ---------------------------------------------------------------------------
# plutus-agent default HERMES_HOME — MUST happen before any hermes module
# import. Many modules elsewhere have a duplicated fallback pattern
# ``os.environ.get("HERMES_HOME", Path.home() / ".hermes")`` that bypasses
# our plutus_constants default. By setting HERMES_HOME in the env *here*,
# all those fallbacks resolve to the right path for the plutus-agent fork
# (``~/.plutus-agent/``) rather than the upstream default (``~/.plutus-agent/``).
# Respects an externally-set HERMES_HOME (CI, docker, dev override).
# Profile override below may further refine HERMES_HOME if -p is passed.
# ---------------------------------------------------------------------------
# honor PLUTUS_HOME (renamed from HERMES_HOME in the plutus-agent fork);
# legacy HERMES_HOME still respected as a fallback / migration bridge.
if not os.environ.get("HERMES_HOME") and os.environ.get("PLUTUS_HOME"):
    os.environ["HERMES_HOME"] = os.environ["PLUTUS_HOME"]
os.environ.setdefault("HERMES_HOME", str(Path.home() / ".plutus-agent"))
...
# Falls back to ~/.plutus-agent/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    if profile_name is not None:
        try:
            from harness.cli.profiles import resolve_profile_env
...
try:
    from harness.log import setup_logging as _setup_logging
...
try:
    from harness.cli.config import load_config as _load_config_early
    from harness.constants import apply_ipv4_preference as _apply_ipv4
...
from harness.constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from harness.cli.config import get_env_path, get_hermes_home, load_config
    from harness.cli.auth import get_auth_status
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'plutus sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'plutus setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from harness.cli.banner import prefetch_update_check
    ...
    # user's ~/.plutus-agent/config.yaml and return built-in defaults. Set BEFORE
    ...
    from harness.repl import main as cli_main
...
def cmd_gateway(args):
    ...
    from harness.cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Plutus?")
        print()
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: plutus gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  plutus gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '◆ plutus-agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  plutus gateway")
        ...
        print()
        print("  Or install as a service: plutus gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'plutus whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from harness.cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None, *, quick=False):
    """Core provider selection + model picking logic.

    Shared by ``cmd_model`` (``plutus model``) and the setup wizard
    ...
    from harness.cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    if selected_provider == "openrouter":
        ...
        _model_flow_api_key_provider(config, selected_provider, current_model, quick=quick)

    # ── Post-switch cleanup: clear stale OPENAI_BASE_URL ──────────────
    # When the user switches to a named provider (anything except "custom"),
    # a leftover OPENAI_BASE_URL in ~/.plutus-agent/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.plutus-agent/.env if the active provider is not 'custom'.
    ...
    """
    from harness.cli.config import get_env_value, save_env_value, load_config
...
# `plutus model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `plutus model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Plutus only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``plutus model`` flow, then route aux tasks to them here.
    """
    from harness.cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from harness.cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from harness.cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from harness.cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from harness.cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from harness.cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from harness.cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `plutus auth` stores device_code tokens),
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from harness.cli.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to ~/.plutus-agent/config.yaml.
    """
    from harness.cli.auth import (
    ...
    if not status.get("logged_in"):
        try:
            from harness.agent.google_oauth import resolve_project_id_from_env, start_oauth_flow
...
def _model_flow_custom(config):
    ...
    """
    from harness.cli.auth import _save_model_choice, deactivate_provider
    from harness.cli.config import get_env_value, load_config, save_config
    ...
    from harness.cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `plutus model` to set a model.")
...
def _model_flow_copilot(config, current_model=""):
    ...
    from harness.cli.config import save_env_value, load_config, save_config
    from harness.cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from harness.cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from harness.cli.copilot_auth import validate_copilot_token
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from harness.cli.config import get_env_value, save_env_value, load_config, save_config
    from harness.cli.models import fetch_api_models
...
def _model_flow_api_key_provider(config, provider_id, current_model="", *, quick=False):
    ...
    # The endpoint is an advanced knob (self-hosted / proxy / alternate region) —
    # hosted providers like OpenCode Go almost always use the default. Only the
    # FULL setup (and the standalone `plutus model` command) prompt for it; the
    # quick setup silently uses the provider default to stay frictionless.
    current_base = ""
    if base_url_env:
        current_base = get_env_value(base_url_env) or os.getenv(base_url_env, "")
    ...
    if not quick:
        print("  API endpoint (the server requests are sent to) — this is NOT the model.")
        print("  You'll choose the model in the next step. Press Enter to keep the default below.")
        try:
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'plutus model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate plutus-agent CLI with a provider."""
    from harness.cli.auth import login_command
...
def cmd_logout(args):
    ...
    from harness.cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from harness.cli.auth_commands import auth_command
...
def cmd_cron(args):
    """Cron job management."""
    from harness.cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from harness.cli.webhook import webhook_command
...
def cmd_hooks(args):
    ...
    from harness.cli.hooks import hooks_command
    hooks_command(args)
...
def cmd_debug(args):
    ...
    from harness.cli.debug import run_debug
...
def cmd_config(args):
    ...
    from harness.cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from harness.cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from harness.cli.backup import run_backup
...
def cmd_import(args):
    ...
    from harness.cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"plutus-agent v{__version__} ({__release_date__})")
...
def cmd_uninstall(args):
    """Uninstall plutus-agent."""
    _require_tty("uninstall")
    from harness.cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``plutus update --gateway`` so interactive prompts (stash restore,
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``plutus web``).
...
def _update_via_zip(args):
    """Update plutus-agent by downloading a ZIP archive.
...
# =========================================================================
# Fork detection and upstream management for `plutus update`
# =========================================================================
...
def _invalidate_update_cache():
    ...
    ``plutus update``, every profile is now current.
    """
    homes = []
...
class _UpdateOutputStream:
    """Stream wrapper used during ``plutus update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.plutus-agent/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``plutus update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``plutus update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.plutus-agent/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``plutus update --gateway``) the update is already
    ...
    try:
        ...
        # plutus_cli.config.get_hermes_home to simulate setup failure.
        from harness.cli.config import get_hermes_home as _get_hermes_home
...
def cmd_update(args):
    """Update plutus-agent to the latest version.
    ...
    if is_managed():
        managed_error("update plutus-agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_hermes_home added to plutus_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload plutus_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_hermes_home() added since the last release.
        try:
            import importlib
            import harness.constants as _hc
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print(
                    "    Run 'plutus config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: plutus config migrate")
            else:
                print()
                print("Skipped. Run 'plutus config migrate' later to configure.")
        else:
        ...
        # When running as ``plutus update --gateway`` (spawned by the gateway's
        ...
        try:
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: plutus gateway run")
        ...
        # every `plutus update` surfaces the issue until the user migrates.
        try:
            from harness.cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                ...
                print()
                print("    plutus gateway migrate-legacy")
                print()
        ...
        print("  plutus model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``plutus -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `plutus profile` — show current profile status
    ...
    if action == "list":
        ...
        from harness.cli.profiles import profile_exists
        ...
        from harness.cli.profiles import rename_profile
        ...
        from harness.cli.profiles import export_profile
        ...
        from harness.cli.profiles import import_profile
...
def cmd_dashboard(args):
    ...
    from harness.cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from harness.cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    ...
    from harness.cli.logs import tail_log, list_logs
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="plutus",
        description="plutus-agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    hermes                        Start interactive chat
    plutus chat -q "Hello"        Single query mode
    plutus -c                     Resume the most recent session
    plutus -c "my project"        Resume a session by name (latest in lineage)
    plutus --resume <session_id>  Resume a specific session by ID
    plutus setup                  Run setup wizard
    plutus logout                 Clear stored authentication
    plutus auth add <provider>    Add a pooled credential
    plutus auth list              List pooled credentials
    plutus auth remove <p> <t>    Remove pooled credential by index, id, or label
    plutus auth reset <provider>  Clear exhaustion status for a provider
    plutus model                  Select default model
    plutus config                 View configuration
    plutus config edit            Edit config in $EDITOR
    plutus config set model gpt-4 Set a config value
    plutus gateway                Run messaging gateway
    hermes -s hermes-agent-dev,github-auth
    plutus -w                     Start in isolated git worktree
    plutus gateway install        Install gateway background service
    plutus sessions list          List past sessions
    plutus sessions browse        Interactive session picker
    plutus sessions rename ID T   Rename/title a session
...
""",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.plutus-agent/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    parser.add_argument(
        "--ignore-rules",
        action="store_true",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with plutus-agent",
    )
    ...
    chat_parser.add_argument(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.plutus-agent/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    chat_parser.add_argument(
        "--ignore-rules",
        action="store_true",
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure plutus-agent with an interactive wizard. "
        "Run a specific section: plutus setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for plutus-agent CLI",
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of plutus-agent components",
    )
    ...
    add_setup_status_subparser(subparsers)
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.plutus-agent/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.plutus-agent/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with plutus-agent setup",
    )
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for plutus-agent. Use 'hermes debug share' to "
...
""",
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage plutus-agent configuration",
    )
    ...
    def cmd_pairing(args):
        from harness.cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.plutus-agent/skills/.bundled_manifest) "
            "so future 'plutus update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from harness.cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from harness.cli.skills_hub import skills_command
    ...
    def cmd_plugins(args):
        from harness.cli.plugins_cmd import plugins_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'plutus tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # plutus tools list [--platform cli]
    ...
    # plutus tools disable <name...> [--platform cli]
    ...
    # plutus tools enable <name...> [--platform cli]
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch plutus --resume <id> by replacing the current process
            ...
            if hermes_bin:
                ...
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "harness.cli.main", "--resume", selected_id],
                )
    ...
    def cmd_claw(args):
        from harness.cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update plutus-agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall plutus-agent",
        description="Remove plutus-agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the plutus-agent web dashboard for managing config, API keys, and sessions",
    )
...
""",
    ...
    # e.g. ``plutus -c Pokemon Agent Dev`` → ``plutus -c 'Pokemon Agent Dev'``
    ...
    from harness.cli.config import get_container_exec_info
    ...
    # fails (e.g. 'plutus -c model' where 'model' is consumed as the
