# Contributing to plutus-agent

Thank you for contributing to plutus-agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want plutus-agent to work everywhere.
...
Bundled skills (in `skills/`) ship with every plutus-agent install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `plutus skills browse` (labeled "official") and install it with `plutus skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `plutus skills install`.
...
```bash
git clone --recurse-submodules https://github.com/sevnightingale/plutus-agent.git
cd plutus-agent
...
```bash
mkdir -p ~/.plutus-agent/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.plutus-agent/config.yaml
touch ~/.plutus-agent/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.plutus-agent/.env
```
...
├── plutus_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── plutus_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.plutus-agent/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.plutus-agent/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.plutus-agent/`)

| Path | Purpose |
|------|---------|
| `~/.plutus-agent/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.plutus-agent/.env` | API keys and secrets |
| `~/.plutus-agent/auth.json` | OAuth credentials (Nous Portal) |
| `~/.plutus-agent/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.plutus-agent/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.plutus-agent/state.db` | SQLite session database |
| `~/.plutus-agent/sessions/` | JSON session logs |
| `~/.plutus-agent/cron/` | Scheduled job data |
| `~/.plutus-agent/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`plutus_state.py`) with full-text search and unique session titles. JSON logs go to `~/.plutus-agent/sessions/`.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `plutus setup` or update `~/.plutus-agent/.env` locally.
...
- **Test it.** Run `plutus --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.
...
Create `~/.plutus-agent/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `plutus_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `plutus_cli/skin_engine.py` for the full schema and existing skins as examples.
...
- Use [GitHub Issues](https://github.com/NousResearch/plutus-agent/issues)
- Include: OS, Python version, Hermes version (`plutus version`), full error traceback
