...
def codex_auth_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "harness.agent.auxiliary_client._read_codex_access_token",
...
class TestReadCodexAccessToken:
    ...
    def test_pool_without_selected_entry_falls_back_to_auth_store(self, tmp_path, monkeypatch):
        ...
        with patch("harness.agent.auxiliary_client._select_pool_entry", return_value=(True, None)), \
             patch("harness.cli.auth._read_codex_tokens", return_value={
    ...
    def test_missing_returns_none(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HERMES_HOME", str(hermes_home))
        with patch("harness.agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            result = _read_codex_access_token()
    ...
    def test_malformed_json_returns_none(self, tmp_path):
        ...
        with patch("harness.agent.auxiliary_client.Path.home", return_value=tmp_path):
            result = _read_codex_access_token()
        assert result is None
...
class TestAnthropicOAuthFlag:
    ...
    def test_oauth_token_sets_flag(self, monkeypatch):
        ...
        with patch("harness.agent.anthropic_adapter.build_anthropic_client") as mock_build:
            mock_build.return_value = MagicMock()
            from harness.agent.auxiliary_client import _try_anthropic, AnthropicAuxiliaryClient
    ...
    def test_api_key_no_oauth_flag(self, monkeypatch):
        ...
        with patch("harness.agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-api03-testkey1234"), \
             patch("harness.agent.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("harness.agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from harness.agent.auxiliary_client import _try_anthropic, AnthropicAuxiliaryClient
            client, model = _try_anthropic()
            assert client is not None
    ...
    def test_pool_entry_takes_priority_over_legacy_resolution(self):
        ...
        with (
            patch("harness.agent.auxiliary_client.load_pool", return_value=_Pool()),
            patch("harness.agent.anthropic_adapter.resolve_anthropic_token", side_effect=AssertionError("legacy path should not run")),
            patch("harness.agent.anthropic_adapter.build_anthropic_client", return_value=MagicMock()) as mock_build,
        ):
            from harness.agent.auxiliary_client import _try_anthropic
...
class TestTryCodex:
    def test_pool_without_selected_entry_falls_back_to_auth_store(self):
        with (
            patch("harness.agent.auxiliary_client._select_pool_entry", return_value=(True, None)),
            patch("harness.agent.auxiliary_client._read_codex_access_token", return_value="codex-auth-token"),
            patch("harness.agent.auxiliary_client.OpenAI") as mock_openai,
        ):
            mock_openai.return_value = MagicMock()
            from harness.agent.auxiliary_client import _try_codex
...
class TestExpiredCodexFallback:
    ...
    def test_expired_codex_falls_through_to_next(self, tmp_path, monkeypatch):
        ...
        with patch("harness.agent.anthropic_adapter.build_anthropic_client") as mock_build:
            mock_build.return_value = MagicMock()
            from harness.agent.auxiliary_client import _resolve_auto, AnthropicAuxiliaryClient
    ...
    def test_expired_codex_custom_endpoint_wins(self, tmp_path, monkeypatch):
        ...
        with patch("harness.agent.auxiliary_client._resolve_custom_runtime",
    ...
    def test_hermes_oauth_file_sets_oauth_flag(self, monkeypatch):
        ...
        with patch("harness.agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat-hermes-token"), \
             patch("harness.agent.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("harness.agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from harness.agent.auxiliary_client import _try_anthropic, AnthropicAuxiliaryClient
...
class TestExplicitProviderRouting:
    ...
    def test_explicit_anthropic_api_key(self, monkeypatch):
        ...
        with patch("harness.agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-api-regular-key"), \
             patch("harness.agent.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("harness.agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
    ...
    def test_explicit_openrouter_pool_exhausted_logs_precise_warning(self, monkeypatch, caplog):
        ...
        with patch("harness.agent.auxiliary_client._select_pool_entry", return_value=(True, None)):
            with caplog.at_level(logging.WARNING, logger="harness.agent.auxiliary_client"):
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            patch("harness.agent.auxiliary_client.load_pool", return_value=_Pool()),
            patch("harness.agent.auxiliary_client.OpenAI"),
            patch("harness.cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from harness.agent.auxiliary_client import _try_codex

            client, model = _try_codex()

        from harness.agent.auxiliary_client import CodexAuxiliaryClient
...
class TestNousAuxiliaryRefresh:
    def test_try_nous_prefers_runtime_credentials(self):
        ...
        with (
            patch("harness.agent.auxiliary_client._read_nous_auth", return_value={"access_token": "stale-token"}),
            patch("harness.agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("harness.cli.models.get_nous_recommended_aux_model", return_value=None),
            patch("harness.agent.auxiliary_client.OpenAI") as mock_openai,
        ):
            from harness.agent.auxiliary_client import _try_nous
    ...
    def test_try_nous_uses_portal_recommendation_for_text(self):
        ...
        fresh_base = "https://inference-api.nousresearch.com/v1"
        with (
            patch("harness.agent.auxiliary_client._read_nous_auth", return_value={"access_token": "***"}),
            patch("harness.agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("harness.cli.models.get_nous_recommended_aux_model", return_value="minimax/minimax-m2.7") as mock_rec,
            patch("harness.agent.auxiliary_client.OpenAI") as mock_openai,
        ):
            from harness.agent.auxiliary_client import _try_nous
    ...
    def test_try_nous_uses_portal_recommendation_for_vision(self):
        ...
        with (
            patch("harness.agent.auxiliary_client._read_nous_auth", return_value={"access_token": "***"}),
            patch("harness.agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("harness.cli.models.get_nous_recommended_aux_model", return_value="google/gemini-3-flash-preview") as mock_rec,
            patch("harness.agent.auxiliary_client.OpenAI"),
        ):
            from harness.agent.auxiliary_client import _try_nous
    ...
    def test_try_nous_falls_back_when_recommendation_lookup_raises(self):
        ...
        with (
            patch("harness.agent.auxiliary_client._read_nous_auth", return_value={"access_token": "***"}),
            patch("harness.agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("harness.cli.models.get_nous_recommended_aux_model", side_effect=RuntimeError("portal down")),
            patch("harness.agent.auxiliary_client.OpenAI"),
        ):
            from harness.agent.auxiliary_client import _try_nous
            client, model = _try_nous()
...
class TestGetProviderChain:
    ...
    def test_picks_up_patched_functions(self):
        ...
        with patch("harness.agent.auxiliary_client._try_openrouter", sentinel):
            chain = _get_provider_chain()
...
class TestTryPaymentFallback:
    ...
    def test_skips_failed_provider(self):
        ...
        with patch("harness.agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("harness.agent.auxiliary_client._try_nous", return_value=(mock_client, "nous-model")), \
             patch("harness.agent.auxiliary_client._read_main_provider", return_value="openrouter"):
    ...
    def test_returns_none_when_no_fallback(self):
        with patch("harness.agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("harness.agent.auxiliary_client._try_nous", return_value=(None, None)), \
             patch("harness.agent.auxiliary_client._try_custom_endpoint", return_value=(None, None)), \
             patch("harness.agent.auxiliary_client._try_codex", return_value=(None, None)), \
             patch("harness.agent.auxiliary_client._resolve_api_key_provider", return_value=(None, None)), \
             patch("harness.agent.auxiliary_client._read_main_provider", return_value="openrouter"):
    ...
    def test_codex_alias_maps_to_chain_label(self):
        ...
        mock_client = MagicMock()
        with patch("harness.agent.auxiliary_client._try_openrouter", return_value=(mock_client, "or-model")), \
             patch("harness.agent.auxiliary_client._try_codex", return_value=(None, None)), \
             patch("harness.agent.auxiliary_client._read_main_provider", return_value="openai-codex"):
    ...
    def test_skips_to_codex_when_or_and_nous_fail(self):
        ...
        with patch("harness.agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("harness.agent.auxiliary_client._try_nous", return_value=(None, None)), \
             patch("harness.agent.auxiliary_client._try_custom_endpoint", return_value=(None, None)), \
             patch("harness.agent.auxiliary_client._try_codex", return_value=(mock_codex, "gpt-5.2-codex")), \
             patch("harness.agent.auxiliary_client._read_main_provider", return_value="openrouter"):
            client, model, label = _try_payment_fallback("openrouter")
...
class TestCallLlmPaymentFallback:
    ...
    def test_non_payment_error_not_caught(self, monkeypatch):
        ...
        with patch("harness.agent.auxiliary_client._get_cached_client",
                    return_value=(primary_client, "google/gemini-3-flash-preview")), \
             patch("harness.agent.auxiliary_client._resolve_task_provider_model",
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from harness.cli.auth import ProviderConfig
    ...
    monkeypatch.setattr("harness.agent.auxiliary_client._try_anthropic", mock_try_anthropic)
    monkeypatch.setattr("harness.cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
        "harness.cli.auth.is_provider_explicitly_configured",
        lambda pid: False,
    )

    from harness.agent.auxiliary_client import _resolve_api_key_provider
...
class TestIsConnectionError:
    ...
    def test_connection_refused(self):
        from harness.agent.auxiliary_client import _is_connection_error
...
class TestKimiTemperatureOmitted:
    ...
    def test_kimi_models_omit_temperature(self, model):
        ...
        from harness.agent.auxiliary_client import _build_call_kwargs
    ...
    def test_sync_call_omits_temperature(self):
        ...
        with patch(
            "harness.agent.auxiliary_client._get_cached_client",
...
class TestStaleBaseUrlWarning:
    ...
    def test_warns_when_openai_base_url_set_with_named_provider(self, monkeypatch, caplog):
        ...
        import harness.agent.auxiliary_client as mod
        # Reset the module-level flag so the warning fires
        monkeypatch.setattr(mod, "_stale_base_url_warned", False)
        monkeypatch.setenv("OPENAI_BASE_URL", "http://localhost:11434/v1")
        monkeypatch.setenv("OPENROUTER_API_KEY", "sk-or-test")

        with patch("harness.agent.auxiliary_client._read_main_provider", return_value="openrouter"), \
             patch("harness.agent.auxiliary_client._read_main_model", return_value="google/gemini-flash"), \
             caplog.at_level(logging.WARNING, logger="harness.agent.auxiliary_client"):
            _resolve_auto()
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("harness.cli.config.load_config", return_value=config), patch(
            "harness.agent.auxiliary_client._get_cached_client",
    ...
    def test_no_warning_when_provider_is_custom(self, monkeypatch, caplog):
        ...
        with patch("harness.agent.auxiliary_client._read_main_provider", return_value="custom"), \
             patch("harness.agent.auxiliary_client._read_main_model", return_value="llama3"), \
             patch("harness.agent.auxiliary_client._resolve_custom_runtime",
    ...
    def test_no_warning_when_provider_is_named_custom(self, monkeypatch, caplog):
        ...
        with patch("harness.agent.auxiliary_client._read_main_provider", return_value="custom:ollama-local"), \
             patch("harness.agent.auxiliary_client._read_main_model", return_value="llama3"), \
             patch("harness.agent.auxiliary_client.resolve_provider_client",
...
class TestAnthropicCompatImageConversion:
    ...
    def test_known_providers_detected(self):
        from harness.agent.auxiliary_client import _is_anthropic_compat_endpoint
    ...
    def test_base64_image_converted(self):
        from harness.agent.auxiliary_client import _convert_openai_images_to_anthropic
        messages = [{
            "role": "user",
            "content": [
