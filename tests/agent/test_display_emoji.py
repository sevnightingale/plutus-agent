...
class TestGetToolEmoji:
    ...
    def test_returns_registry_emoji_when_no_skin(self):
        ...
        with mock_patch("harness.agent.display._get_skin", return_value=None), \
             mock_patch("harness.agent.display.registry", mock_registry, create=True):
        ...
        with mock_patch("harness.agent.display._get_skin", return_value=None):
            ...
            with mock_patch.dict("sys.modules", {}):
                ...
                with mock_patch.dict(sys.modules, {"harness.tools.registry": mock_module}):
    ...
    def test_skin_override_takes_precedence(self):
        ...
        with mock_patch("harness.agent.display._get_skin", return_value=skin):
    ...
    def test_skin_empty_dict_falls_through(self):
        ...
        mock_module.registry = mock_reg
        with mock_patch("harness.agent.display._get_skin", return_value=skin), \
             mock_patch.dict(sys.modules, {"harness.tools.registry": mock_module}):
            result = get_tool_emoji("terminal")
            assert result == "💻"
...
class TestSkinConfigToolEmojis:
    ...
    def test_skin_config_has_tool_emojis_field(self):
        from harness.cli.skin_engine import SkinConfig
    ...
    def test_build_skin_config_includes_tool_emojis(self):
        from harness.cli.skin_engine import _build_skin_config
        data = {
            "name": "custom",
