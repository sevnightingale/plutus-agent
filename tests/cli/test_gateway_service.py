...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_restart_drains_running_gateway_before_kickstart(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "harness.gateway.status.get_running_pid",
            lambda: 321,
        )
...
class TestGatewaySystemServiceRouting:
    def test_systemd_restart_self_requests_graceful_restart_and_waits(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr("harness.gateway.status.get_running_pid", fake_get_pid)
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: plutus gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup plutus gateway" in out
...
class TestSystemUnitHermesHome:
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        # plutus-agent uses ~/.plutus-agent as the user data dir (not the
        # upstream ~/.hermes); _hermes_home_for_target_user derives the dir
        # name from get_hermes_home() so this works on either fork.
        assert 'HERMES_HOME=/home/alice/.plutus-agent' in unit
        assert '/root/.plutus-agent' not in unit
    ...
    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("HERMES_HOME", "/root/.plutus-agent/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'HERMES_HOME=/home/alice/.plutus-agent/profiles/coder' in unit
...
class TestHermesHomeForTargetUser:
    ...
    def test_remaps_default_home(self, monkeypatch):
        ...
        # plutus-agent fork uses ~/.plutus-agent (not upstream's ~/.hermes)
        assert result == "/home/alice/.plutus-agent"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._hermes_home_for_target_user("/home/alice")
        assert result == "/home/alice/.plutus-agent/profiles/coder"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "plutus gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.plutus-agent/profiles/mybot should return '--profile mybot'."""
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.plutus-agent/profiles/mybot/subdir should NOT match — too deep."""
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".plutus-agent" / "hermes-agent"
        ...
        monkeypatch.setenv("HERMES_HOME", str(root_home / ".plutus-agent"))
        monkeypatch.setattr(gateway_cli, "get_hermes_home", lambda: root_home / ".plutus-agent")
        ...
        assert "WorkingDirectory=/home/alice/.plutus-agent/hermes-agent" in unit
...
class TestDockerAwareGateway:
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "plutus gateway run" in out
...
class TestLegacyHermesUnitDetection:
    ...
    _OUR_UNIT_TEXT = (
        ...
        "ExecStart=/usr/bin/python -m plutus_cli.main gateway run --replace\n"
    )
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "plutus gateway migrate-legacy" in out
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo plutus gateway migrate-legacy" in out
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        if project_root is None:
            import harness.cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "harness.cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "harness.cli.main", "gateway", "status", "-l", "--help"],
