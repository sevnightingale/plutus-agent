"""Tests for plutus_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestSaveDisabledSkills:
    @patch("harness.cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from harness.cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("harness.cli.config.load_config")
    def test_globally_disabled(self, mock_load):
        mock_load.return_value = {"skills": {"disabled": ["bad-skill"]}}
        from harness.tools.skills_tool import _is_skill_disabled
...
class TestFindAllSkillsFiltering:
    @patch("harness.tools.skills_tool._get_disabled_skill_names", return_value={"my-skill"})
    @patch("harness.tools.skills_tool.skill_matches_platform", return_value=True)
    @patch("harness.tools.skills_tool.SKILLS_DIR")
    ...
    @patch("harness.tools.skills_tool._get_disabled_skill_names", return_value=set())
    @patch("harness.tools.skills_tool.skill_matches_platform", return_value=True)
    @patch("harness.tools.skills_tool.SKILLS_DIR")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from harness.cli.skills_config import _get_categories
        skills = [
