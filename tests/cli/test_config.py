"""Tests for plutus_cli configuration management."""
...
class TestGetHermesHome:
    def test_default_path(self):
        # plutus-agent: default home is ~/.plutus-agent (not upstream's ~/.hermes).
        # Phase 3 user-data-dir isolation.
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HOME", None)
            home = get_hermes_home()
            assert home == Path.home() / ".plutus-agent"
...
class TestSaveConfigAtomicity:
    ...
    def test_no_partial_write_on_crash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            with patch("harness.utils.yaml.dump", side_effect=OSError("disk full")):
                try:
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from harness.cli.config import ENV_VARS_BY_VERSION
        all_vars = []
