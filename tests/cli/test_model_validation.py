"""Tests for provider-aware `/model` validation in plutus_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("harness.cli.models.fetch_api_models", return_value=api_models), \
         patch("harness.cli.models.probe_api_models", return_value=probe_payload):
...
class TestCuratedModelsForProvider:
    def test_openrouter_returns_curated_list(self):
        with patch(
            "harness.cli.models.fetch_openrouter_models",
            return_value=[
...
class TestProviderModelIds:
    ...
    def test_copilot_prefers_live_catalog(self):
        with patch("harness.cli.auth.resolve_api_key_provider_credentials", return_value={"api_key": "gh-token"}), \
             patch("harness.cli.models._fetch_github_models", return_value=["gpt-5.4", "claude-sonnet-4.6"]):
    ...
    def test_copilot_acp_falls_back_to_copilot_defaults(self):
        with patch("harness.cli.auth.resolve_api_key_provider_credentials", side_effect=Exception("no token")), \
             patch("harness.cli.models._fetch_github_models", return_value=None):
...
class TestFetchApiModels:
    ...
    def test_returns_none_on_network_error(self):
        with patch("harness.cli.models.urllib.request.urlopen", side_effect=Exception("timeout")):
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("harness.cli.models.urllib.request.urlopen", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("harness.cli.models.urllib.request.urlopen", return_value=_Resp()) as mock_urlopen:
...
class TestValidateApiFallback:
    ...
    def test_known_model_accepted_via_catalog_when_api_down(self):
        ...
        with patch(
            "harness.cli.models.provider_model_ids",
    ...
    def test_unknown_provider_soft_accepted_when_api_down(self):
        ...
        with patch("harness.cli.models.provider_model_ids", return_value=[]):
...
class TestValidateCodexAutoCorrection:
    ...
    def test_missing_dash_auto_corrects(self):
        ...
        with patch("harness.cli.models.provider_model_ids", return_value=codex_models):
...
class TestProbeApiModelsUserAgent:
    ...
    def test_probe_sends_hermes_user_agent(self):
        ...
        with patch(
            "harness.cli.models.urllib.request.urlopen",
