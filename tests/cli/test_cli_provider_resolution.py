...
from harness.cli import main as hermes_main
...
# mock patches that target "harness.tools.file_tools._get_file_ops" etc.
# ---------------------------------------------------------------------------
...
def _import_cli():
    for name in list(sys.modules):
        if name in ("harness.repl", "harness.run_agent", "harness.tools") or name.startswith("harness.tools."):
            sys.modules.pop(name, None)
    ...
    return importlib.import_module("harness.repl")
...
def test_hermes_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    ...
    monkeypatch.setattr("harness.cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("harness.cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("harness.cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("harness.cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "harness.cli.auth.get_provider_auth_state",
        lambda provider: {"access_token": "nous-token"},
    )
    monkeypatch.setattr(
        "harness.cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "harness.cli.auth.fetch_nous_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("harness.cli.auth._prompt_model_selection", lambda model_ids, current_model="", pricing=None, **kw: "claude-opus-4-6")
    monkeypatch.setattr("harness.cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("harness.cli.auth._update_config_for_provider", lambda provider, url: None)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("harness.cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("harness.cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("harness.cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("harness.cli.auth.resolve_provider", _resolve_provider)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "harness.cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("harness.cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("harness.cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("harness.cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("harness.cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "harness.cli.models.probe_api_models",
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    ...
    monkeypatch.setattr("harness.cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("harness.cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("harness.cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("harness.cli.auth.get_provider_auth_state", lambda provider_id: None)
    ...
    monkeypatch.setattr("harness.cli.auth._login_nous", _fake_login)
...
def test_auto_provider_name_localhost():
    from harness.cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from harness.cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "harness.cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("harness.cli.config.save_config", _save)
