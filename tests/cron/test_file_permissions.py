...
class TestCronFilePermissions(unittest.TestCase):
    ...
    @patch("harness.cron.jobs.CRON_DIR")
    @patch("harness.cron.jobs.OUTPUT_DIR")
    @patch("harness.cron.jobs.JOBS_FILE")
    def test_ensure_dirs_sets_0700(self, mock_jobs_file, mock_output, mock_cron):
        ...
        with patch("harness.cron.jobs.CRON_DIR", cron_dir), \
             patch("harness.cron.jobs.OUTPUT_DIR", output_dir):
            from harness.cron.jobs import ensure_dirs
            ensure_dirs()
    ...
    def test_save_jobs_sets_0600(self, mock_jobs_file, mock_output, mock_cron):
        ...
        with patch("harness.cron.jobs.CRON_DIR", cron_dir), \
             patch("harness.cron.jobs.OUTPUT_DIR", output_dir), \
             patch("harness.cron.jobs.JOBS_FILE", jobs_file):
            from harness.cron.jobs import save_jobs
    ...
    def test_save_job_output_sets_0600(self):
        ...
        with patch("harness.cron.jobs.OUTPUT_DIR", output_dir), \
             patch("harness.cron.jobs.CRON_DIR", Path(self.tmpdir)), \
             patch("harness.cron.jobs.ensure_dirs"):
            output_dir.mkdir(parents=True, exist_ok=True)
            from harness.cron.jobs import save_job_output
...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("harness.cli.config.get_config_path", return_value=config_path), \
             patch("harness.cli.config.ensure_hermes_home"):
    ...
    def test_save_env_value_sets_0600(self):
        ...
        with patch("harness.cli.config.get_env_path", return_value=env_path), \
             patch("harness.cli.config.ensure_hermes_home"):
    ...
    def test_ensure_hermes_home_sets_0700(self):
        ...
        with patch("harness.cli.config.get_hermes_home", return_value=home):
