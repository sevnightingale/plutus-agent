...
class TestXAIImageGenProvider:
    def test_name(self):
        from harness.plugins.image_gen.xai import XAIImageGenProvider
...
class TestConfig:
    def test_default_model(self):
        from harness.plugins.image_gen.xai import _resolve_model

        model_id, meta = _resolve_model()
        assert model_id == "grok-imagine-image"

    def test_default_resolution(self):
        from harness.plugins.image_gen.xai import _resolve_resolution
...
class TestGenerate:
    ...
    def test_successful_generation(self):
        ...
        with patch("harness.plugins.image_gen.xai.requests.post", return_value=mock_resp):
            with patch("harness.plugins.image_gen.xai.save_b64_image", return_value="/tmp/test.png"):
                provider = XAIImageGenProvider()
    ...
    def test_timeout(self):
        ...
        with patch("harness.plugins.image_gen.xai.requests.post", side_effect=req_lib.Timeout()):
            provider = XAIImageGenProvider()
            result = provider.generate(prompt="test")
    ...
    def test_auth_header(self):
        ...
        with patch("harness.plugins.image_gen.xai.requests.post", return_value=mock_resp) as mock_post:
            provider = XAIImageGenProvider()
...
class TestRegistration:
    def test_register(self):
        from harness.plugins.image_gen.xai import XAIImageGenProvider, register
