...
* ``--ignore-user-config`` → skip ``~/.plutus-agent/config.yaml`` in ``load_cli_config()``
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``plutus_cli/main.py`` must set
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real plutus_cli/main.py parser accepts both flags.
