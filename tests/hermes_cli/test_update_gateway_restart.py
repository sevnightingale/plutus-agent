...
from harness.cli.main import cmd_update
...
def _no_restart_verify_sleep(monkeypatch):
    """harness/cli/main.py uses time.sleep(3) after systemctl restart to
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_falls_back_to_systemctl_restart_when_sigusr1_times_out(
        ...
        monkeypatch.setattr(
            "harness.cli.gateway._graceful_restart_via_sigusr1",
    ...
    def test_update_no_gateway_running_skips_restart(
        ...
        with patch("harness.gateway.status.get_running_pid", return_value=None):
            cmd_update(mock_args)
