...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from plutus_cli.debug import _upload_paste_rs
        ...
        with patch("plutus_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "plutus_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from plutus_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from plutus_cli.debug import upload_to_pastebin

        with patch("plutus_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("plutus_cli.debug._upload_paste_rs",
             ...
             patch("plutus_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, hermes_home):
        from plutus_cli.debug import _capture_log_snapshot
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, hermes_home):
        from plutus_cli.debug import collect_debug_report

        with patch("plutus_cli.dump.run_dump") as mock_dump:
    ...
    def test_report_includes_agent_log(self, hermes_home):
        ...
        with patch("plutus_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, hermes_home, capsys):
        ...
        with patch("plutus_cli.dump.run_dump"), \
             patch("plutus_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("plutus_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, hermes_home, capsys):
        ...
        with patch("plutus_cli.dump.run_dump"), \
             patch(
                 "plutus_cli.debug._sweep_expired_pastes",
    ...
    def test_share_keeps_report_and_full_log_on_same_snapshot(self, hermes_home, capsys):
        ...
        from plutus_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("plutus_cli.dump.run_dump"), \
             patch("plutus_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("plutus_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from plutus_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from plutus_cli.debug import delete_paste
        ...
        with patch("plutus_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    def test_does_not_spawn_subprocess(self, hermes_home):
        ...
        import ast
        import inspect
        from plutus_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, hermes_home):
        ...
        from plutus_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_merges_with_existing_pending(self, hermes_home):
        ...
        from plutus_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, hermes_home):
        from plutus_cli.debug import _sweep_expired_pastes
    ...
    def test_sweep_deletes_expired_entries(self, hermes_home):
        ...
        with patch("plutus_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, hermes_home):
        from plutus_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("plutus_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, hermes_home):
        ...
        with patch("plutus_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from plutus_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("plutus_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("plutus_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, hermes_home, capsys):
        ...
        with patch("plutus_cli.dump.run_dump"), \
             ...
             patch("plutus_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, hermes_home, capsys):
        ...
        with patch("plutus_cli.dump.run_dump"), \
             patch("plutus_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("plutus_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
