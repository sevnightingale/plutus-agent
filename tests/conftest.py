...
2. **Isolated HERMES_HOME.** HERMES_HOME points to a per-test tempdir so
   code reading ``~/.plutus-agent/*`` via ``get_hermes_home()`` can't see the
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    reads ``~/.plutus-agent/*`` can't touch the real one, and pins TZ/LANG so
    ...
    #    ``~/.plutus-agent/*`` via ``get_hermes_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.plutus-agent/*`` via
    ...
    #    ~/.plutus-agent/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import harness.cli.plugins as _plugins_mod
...
def _reset_module_state():
    ...
    try:
        from harness.tools import approval as _approval_mod
    ...
    try:
        from harness.tools import interrupt as _interrupt_mod
    ...
    try:
        from harness.gateway import session_context as _sc_mod
        for _cv in (
    ...
    try:
        from harness.tools import env_passthrough as _envp_mod
    ...
    try:
        from harness.tools import credential_files as _credf_mod
    ...
    try:
        from harness.tools import file_tools as _ft_mod
