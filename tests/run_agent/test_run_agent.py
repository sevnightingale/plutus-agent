...
import pytest
from harness.agent.codex_responses_adapter import _chat_messages_to_responses_input, _normalize_codex_response, _preflight_codex_input_items

import harness.run_agent as run_agent
from harness.run_agent import AIAgent
from harness.agent.error_classifier import FailoverReason
...
def agent():
    ...
    with (
        patch(
            "harness.run_agent.get_tool_definitions", return_value=_make_tool_defs("web_search")
        ),
...
class TestInit:
    def test_anthropic_base_url_accepted(self):
        ...
        with (
            ...
            patch("harness.run_agent.check_toolset_requirements", return_value={}),
            patch("harness.agent.anthropic_adapter._anthropic_sdk") as mock_anthropic,
        ):
            agent = AIAgent(
                api_key="test-key-1234567890",
    ...
    def test_prompt_caching_native_anthropic(self):
        ...
        with (
            patch("harness.run_agent.get_tool_definitions", return_value=[]),
            patch("harness.run_agent.check_toolset_requirements", return_value={}),
            patch("harness.agent.anthropic_adapter._anthropic_sdk"),
        ):
            a = AIAgent(
                api_key="test-key-1234567890",
                base_url="https://api.anthropic.com/v1/",
    ...
    def test_valid_tool_names_populated(self):
        ...
        with (
            patch("harness.run_agent.get_tool_definitions", return_value=tools),
            patch("harness.run_agent.check_toolset_requirements", return_value={}),
            patch("harness.run_agent.OpenAI"),
        ):
            a = AIAgent(
...
class TestInterrupt:
    def test_interrupt_sets_flag(self, agent):
        with patch("harness.run_agent._set_interrupt"):
            agent.interrupt()
...
class TestBuildSystemPrompt:
    ...
    def test_memory_guidance_when_memory_tool_loaded(self, agent_with_memory_tool):
        from harness.agent.prompt_builder import MEMORY_GUIDANCE
    ...
    def test_skills_prompt_derives_available_toolsets_from_loaded_tools(self):
        ...
        with (
            patch("harness.run_agent.get_tool_definitions", return_value=tools),
            patch(
                "harness.run_agent.check_toolset_requirements",
                side_effect=AssertionError("should not re-check toolset requirements"),
            ),
            patch("harness.run_agent.get_toolset_for_tool", create=True, side_effect=toolset_map.get),
            patch("harness.run_agent.build_skills_system_prompt", return_value="SKILLS_PROMPT") as mock_skills,
            patch("harness.run_agent.OpenAI"),
        ):
            agent = AIAgent(
                api_key="test-k...7890",
...
class TestToolUseEnforcementConfig:
    ...
    def test_auto_injects_for_gpt(self):
        from harness.agent.prompt_builder import TOOL_USE_ENFORCEMENT_GUIDANCE
...
class TestExecuteToolCalls:
    def test_single_tool_executed(self, agent):
        ...
        messages = []
        with patch(
            "harness.run_agent.handle_function_call", return_value="search result"
        ) as mock_hfc:
    ...
    def test_invalid_json_args_defaults_empty(self, agent):
        ...
        mock_msg = _mock_assistant_msg(content="", tool_calls=[tc])
        messages = []
        with patch("harness.run_agent.handle_function_call", return_value="ok") as mock_hfc:
            agent._execute_tool_calls(mock_msg, messages, "task-1")
    ...
    def test_result_truncation_over_100k(self, agent, tmp_path, monkeypatch):
        ...
        with patch("harness.run_agent.handle_function_call", return_value=big_result):
            agent._execute_tool_calls(mock_msg, messages, "task-1")
    ...
    def test_quiet_tool_output_suppressed_when_progress_callback_present(self, agent):
        ...
        with patch("harness.run_agent.handle_function_call", return_value="search result"), \
    ...
    def test_run_conversation_suppresses_retry_noise_in_parseable_quiet_mode(self, agent):
        ...
        with patch("harness.run_agent.time.sleep", return_value=None):
...
class TestConcurrentToolExecution:
    ...
    def test_concurrent_executes_all_tools(self, agent):
        ...
        with patch("harness.run_agent.handle_function_call", side_effect=fake_handle):
    ...
    def test_invoke_tool_dispatches_to_handle_function_call(self, agent):
        ...
        with patch("harness.run_agent.handle_function_call", return_value="result") as mock_hfc:
    ...
    def test_sequential_tool_callbacks_fire_in_order(self, agent):
        ...
        with patch("harness.run_agent.handle_function_call", return_value='{"success": true}'):
    ...
    def test_concurrent_tool_callbacks_fire_for_each_tool(self, agent):
        ...
        with patch("harness.run_agent.handle_function_call", side_effect=['{"id":1}', '{"id":2}']):
            agent._execute_tool_calls_concurrent(mock_msg, messages, "task-1")
    ...
    def test_invoke_tool_handles_agent_level_tools(self, agent):
        ...
        with patch("harness.tools.todo_tool.todo_tool", return_value='{"ok":true}') as mock_todo:
    ...
    def test_invoke_tool_blocked_returns_error_and_skips_execution(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "harness.cli.plugins.get_pre_tool_call_block_message",
            lambda *args, **kwargs: "Blocked by test policy",
        )
        with patch("harness.tools.todo_tool.todo_tool", side_effect=AssertionError("should not run")) as mock_todo:
            result = agent._invoke_tool("todo", {"todos": []}, "task-1")
    ...
    def test_invoke_tool_blocked_skips_handle_function_call(self, agent, monkeypatch):
        ...
        with patch("harness.run_agent.handle_function_call", side_effect=AssertionError("should not run")):
            result = agent._invoke_tool("web_search", {"q": "test"}, "task-1")
    ...
    def test_blocked_memory_tool_does_not_reset_counter(self, agent, monkeypatch):
        ...
        with patch("harness.tools.memory_tool.memory_tool", side_effect=AssertionError("should not run")):
...
class TestPathsOverlap:
    ...
    def test_same_path_overlaps(self):
        from harness.run_agent import _paths_overlap
...
class TestParallelScopePathNormalization:
    def test_extract_parallel_scope_path_normalizes_relative_to_cwd(self, tmp_path, monkeypatch):
        from harness.run_agent import _extract_parallel_scope_path
    ...
    def test_extract_parallel_scope_path_treats_relative_and_absolute_same_file_as_same_scope(self, tmp_path, monkeypatch):
        from harness.run_agent import _extract_parallel_scope_path, _paths_overlap
    ...
    def test_should_parallelize_tool_batch_rejects_same_file_with_mixed_path_spellings(self, tmp_path, monkeypatch):
        from harness.run_agent import _should_parallelize_tool_batch
...
class TestRunConversation:
    ...
    def test_tool_calls_then_stop(self, agent):
        ...
        with (
            patch("harness.run_agent.handle_function_call", return_value="search result") as mock_handle_function_call,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            patch("harness.run_agent.handle_function_call", return_value="search result"),
            patch("harness.cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_interrupt_breaks_loop(self, agent):
        ...
        with (
            ...
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
            patch("harness.run_agent._set_interrupt"),
            patch.object(
    ...
    def test_context_compression_triggered(self, agent):
        ...
        with (
            patch("harness.run_agent.handle_function_call", return_value="result"),
            patch.object(
    ...
    def test_length_with_tool_calls_returns_partial_without_executing_tools(self, agent):
        ...
        with (
            patch("harness.run_agent.handle_function_call") as mock_handle_function_call,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_truncated_tool_call_retries_once_before_refusing(self, agent):
        ...
        with (
            patch("harness.run_agent.handle_function_call", return_value='{"success":true}') as mock_hfc,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
...
class TestRetryExhaustion:
    ...
    def test_invalid_response_returns_error_not_crash(self, agent):
        ...
        with (
            ...
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
            patch("harness.run_agent.time", self._make_fast_time_mock()),
        ):
            result = agent.run_conversation("hello")
...
class TestNousCredentialRefresh:
    ...
    def test_try_refresh_nous_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "harness.cli.auth.resolve_nous_runtime_credentials", _fake_resolve
        )

        agent.client = _ExistingClient()
        with patch("harness.run_agent.OpenAI", side_effect=_fake_openai):
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "plutus-agent" in agent._cached_system_prompt
...
class TestSafeWriter:
    ...
    def test_write_delegates_normally(self):
        ...
        from harness.run_agent import _SafeWriter
...
class TestSaveSessionLogAtomicWrite:
    def test_uses_shared_atomic_json_helper(self, agent, tmp_path):
        ...
        with patch("harness.run_agent.atomic_json_write", create=True) as mock_atomic_write:
...
class TestBuildApiKwargsAnthropicMaxTokens:
    ...
    def test_max_tokens_passed_to_anthropic(self, agent):
        ...
        with patch("harness.agent.anthropic_adapter.build_anthropic_kwargs") as mock_build:
...
class TestAnthropicImageFallback:
    def test_build_api_kwargs_converts_multimodal_user_image_to_text(self, agent):
        ...
        with (
            patch("harness.tools.vision_tools.vision_analyze_tool", new=AsyncMock(return_value=json.dumps({"success": True, "analysis": "A cat sitting on a chair."}))),
            patch("harness.agent.anthropic_adapter.build_anthropic_kwargs") as mock_build,
        ):
            mock_build.return_value = {"model": "claude-sonnet-4-20250514", "messages": [], "max_tokens": 4096}
    ...
    def test_build_api_kwargs_reuses_cached_image_analysis_for_duplicate_images(self, agent):
        ...
        with (
            patch("harness.tools.vision_tools.vision_analyze_tool", new=mock_vision),
            patch("harness.agent.anthropic_adapter.build_anthropic_kwargs") as mock_build,
        ):
            mock_build.return_value = {"model": "claude-sonnet-4-20250514", "messages": [], "max_tokens": 4096}
            agent._build_api_kwargs(api_messages)
...
class TestFallbackAnthropicProvider:
    ...
    def test_fallback_to_anthropic_sets_api_mode(self, agent):
        ...
        with (
            patch("harness.agent.auxiliary_client.resolve_provider_client", return_value=(mock_client, None)),
            patch("harness.agent.anthropic_adapter.build_anthropic_client") as mock_build,
            patch("harness.agent.anthropic_adapter.resolve_anthropic_token", return_value=None),
        ):
    ...
    def test_fallback_to_anthropic_enables_prompt_caching(self, agent):
        ...
        with (
            patch("harness.agent.auxiliary_client.resolve_provider_client", return_value=(mock_client, None)),
            patch("harness.agent.anthropic_adapter.build_anthropic_client", return_value=MagicMock()),
            patch("harness.agent.anthropic_adapter.resolve_anthropic_token", return_value=None),
        ):
    ...
    def test_fallback_to_openrouter_uses_openai_client(self, agent):
        ...
        with patch("harness.agent.auxiliary_client.resolve_provider_client", return_value=(mock_client, None)):
            result = agent._try_activate_fallback()
...
def test_aiagent_uses_copilot_acp_client():
    with (
        ...
        patch("harness.agent.copilot_acp_client.CopilotACPClient") as mock_acp_client,
    ):
...
class TestAnthropicCredentialRefresh:
    def test_try_refresh_anthropic_client_credentials_rebuilds_client(self):
        ...
        with (
            patch("harness.agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat01-fresh-token"),
            patch("harness.agent.anthropic_adapter.build_anthropic_client", return_value=new_client) as rebuild,
        ):
    ...
    def test_try_refresh_anthropic_client_credentials_returns_false_when_token_unchanged(self):
        ...
        with (
            patch("harness.agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat01-same-token"),
            patch("harness.agent.anthropic_adapter.build_anthropic_client") as rebuild,
        ):
...
class TestOAuthFlagAfterCredentialRefresh:
    ...
    def test_oauth_flag_updates_api_key_to_oauth(self, agent):
        ...
        with (
            patch("harness.agent.anthropic_adapter.resolve_anthropic_token",
                  return_value="sk-ant-setup-oauth-token"),
            patch("harness.agent.anthropic_adapter.build_anthropic_client",
...
class TestMemoryNudgeCounterPersistence:
    ...
    def test_counters_initialized_in_init(self):
        ...
        with patch("harness.run_agent.get_tool_definitions", return_value=[]):
            a = AIAgent(
...
class TestMemoryContextSanitization:
    ...
    def test_sanitize_context_strips_full_block(self):
        ...
        from harness.agent.memory_manager import sanitize_context
