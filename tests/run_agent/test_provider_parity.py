...
def _make_agent(monkeypatch, provider, api_mode="chat_completions", base_url="https://openrouter.ai/api/v1", model=None):
    monkeypatch.setattr("harness.run_agent.get_tool_definitions", lambda **kw: _tool_defs("web_search", "terminal"))
    monkeypatch.setattr("harness.run_agent.check_toolset_requirements", lambda: {})
    monkeypatch.setattr("harness.run_agent.OpenAI", _FakeOpenAI)
    kwargs = dict(
        api_key="test-key",
        base_url=base_url,
        provider=provider,
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_openrouter_always_wins(self, monkeypatch):
        ...
        with patch("harness.agent.auxiliary_client.OpenAI") as mock:
    ...
    def test_nous_when_no_openrouter(self, monkeypatch):
        ...
        from harness.agent.auxiliary_client import get_text_auxiliary_client
        with patch("harness.agent.auxiliary_client._read_nous_auth", return_value={"access_token": "nous-tok"}), \
             patch("harness.agent.auxiliary_client.OpenAI") as mock:
            client, model = get_text_auxiliary_client()
        assert model == "google/gemini-3-flash-preview"
    ...
    def test_custom_endpoint_when_no_nous(self, monkeypatch):
        ...
        from harness.agent.auxiliary_client import get_text_auxiliary_client
        with patch("harness.agent.auxiliary_client._read_nous_auth", return_value=None), \
    ...
    def test_codex_fallback_last_resort(self, monkeypatch):
        ...
        from harness.agent.auxiliary_client import get_text_auxiliary_client, CodexAuxiliaryClient
        with patch("harness.agent.auxiliary_client._read_nous_auth", return_value=None), \
             patch("harness.agent.auxiliary_client._read_codex_access_token", return_value="codex-tok"), \
