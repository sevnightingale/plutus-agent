...
class TestCacheImageFromBytes:
    ...
    def test_caches_valid_jpeg(self, tmp_path, monkeypatch):
        monkeypatch.setattr("harness.gateway.platforms.base.IMAGE_CACHE_DIR", tmp_path / "img")
...
@patch("harness.tools.url_safety.is_safe_url", return_value=True)
    ...
    def test_success_on_first_attempt(self, _mock_safe, tmp_path, monkeypatch):
        ...
        async def run():
            with patch("httpx.AsyncClient", return_value=mock_client):
                from harness.gateway.platforms.base import cache_image_from_url
...
class TestCacheAudioFromUrl:
    ...
    def test_success_on_first_attempt(self, _mock_safe, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("harness.gateway.platforms.base.AUDIO_CACHE_DIR", tmp_path / "audio")
        ...
        async def run():
            with patch("httpx.AsyncClient", return_value=mock_client):
                from harness.gateway.platforms.base import cache_audio_from_url
...
class TestSSRFRedirectGuard:
    ...
    def test_image_blocks_private_redirect(self, tmp_path, monkeypatch):
        ...
        async def run():
            with patch("harness.tools.url_safety.is_safe_url", side_effect=fake_safe), \
    ...
    def test_safe_redirect_allowed(self, tmp_path, monkeypatch):
        ...
        async def run():
            with patch("harness.tools.url_safety.is_safe_url", return_value=True), \
                 patch("httpx.AsyncClient", side_effect=factory):
                from harness.gateway.platforms.base import cache_image_from_url
                return await cache_image_from_url(
...
import harness.gateway.platforms.slack as _slack_mod  # noqa: E402
_slack_mod.SLACK_AVAILABLE = True

from harness.gateway.platforms.slack import SlackAdapter  # noqa: E402
from harness.gateway.config import Platform, PlatformConfig  # noqa: E402
