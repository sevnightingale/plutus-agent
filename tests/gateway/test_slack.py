...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        with patch.object(_slack_mod, "AsyncApp", return_value=mock_app), \
             ...
             patch("harness.gateway.status.acquire_scoped_lock", return_value=(True, None)), \
...
class TestSlackConnectCleanup:
    ...
    async def test_releases_platform_lock_when_auth_fails(self):
        ...
        with patch.object(_slack_mod, "AsyncApp", return_value=mock_app), \
             ...
             patch.dict(os.environ, {"SLACK_APP_TOKEN": "xapp-fake"}), \
             patch("harness.gateway.status.acquire_scoped_lock", return_value=(True, None)), \
             patch("harness.gateway.status.release_scoped_lock") as mock_release:
...
class TestReactions:
    ...
    async def test_reactions_in_message_flow(self, adapter):
        ...
        from harness.gateway.platforms.base import ProcessingOutcome
    ...
    async def test_reactions_failure_outcome(self, adapter):
        ...
        from harness.gateway.platforms.base import MessageEvent, MessageType, SessionSource, ProcessingOutcome
        from harness.gateway.config import Platform
        source = SessionSource(
            platform=Platform.SLACK,
            chat_id="C123",
...
class TestSendImageSSRFGuards:
    ...
    async def test_send_image_blocks_private_redirect_target(self, adapter):
        ...
        with (
            patch("harness.tools.url_safety.is_safe_url", side_effect=fake_is_safe_url),
