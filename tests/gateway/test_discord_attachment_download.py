...
class TestCacheDiscordImage:
    ...
    async def test_prefers_att_read_over_url(self):
        ...
        with patch(
            "harness.gateway.platforms.discord.cache_image_from_bytes",
...
class TestCacheDiscordAudio:
    ...
    async def test_prefers_att_read_over_url(self):
        ...
        with patch(
            "harness.gateway.platforms.discord.cache_audio_from_bytes",
            return_value="/tmp/voice.ogg",
        ) as mock_bytes, patch(
            "harness.gateway.platforms.discord.cache_audio_from_url",
            new_callable=AsyncMock,
        ) as mock_url:
...
class TestCacheDiscordDocument:
    ...
    async def test_fallback_blocked_by_ssrf_guard(self):
        ...
        with patch(
            "harness.gateway.platforms.discord.is_safe_url", return_value=False
    ...
    async def test_fallback_aiohttp_when_safe_url(self):
        ...
        with patch(
            "harness.gateway.platforms.discord.is_safe_url", return_value=True
...
class TestHandleMessageUsesAuthenticatedRead:
    ...
    async def test_image_downloads_via_att_read_not_url(self, monkeypatch):
        ...
        with patch(
            ...
            monkeypatch.setattr(
                "harness.gateway.platforms.discord.discord.DMChannel",
                _FakeDMChannel,
            )
            chan = _FakeDMChannel()
            msg = SimpleNamespace(
