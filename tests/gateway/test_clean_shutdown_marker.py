...
from harness.gateway.config import GatewayConfig, Platform, PlatformConfig, SessionResetPolicy
from harness.gateway.session import SessionEntry, SessionSource, SessionStore
...
class TestCleanShutdownMarker:
    ...
    def test_marker_written_on_graceful_stop(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("harness.gateway.run._hermes_home", tmp_path)
        ...
        with patch("harness.gateway.run.GatewayRunner._drain_active_agents", new_callable=AsyncMock, return_value=([], False)), \
             patch("harness.gateway.run.GatewayRunner._finalize_shutdown_agents"), \
             patch("harness.gateway.run.GatewayRunner._update_runtime_status"), \
             patch("harness.gateway.status.remove_pid_file"), \
             patch("harness.tools.process_registry.process_registry") as mock_proc_reg, \
             patch("harness.tools.terminal_tool.cleanup_all_environments"), \
             patch("harness.tools.browser_tool.cleanup_all_browsers"):
