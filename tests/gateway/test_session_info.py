...
def _patch_info(tmp_path, config_yaml, model, runtime):
    ...
    return (
        patch("harness.gateway.run._hermes_home", tmp_path),
        patch("harness.gateway.run._resolve_gateway_model", return_value=model),
        patch("harness.gateway.run._resolve_runtime_agent_kwargs", return_value=runtime),
    )
...
class TestFormatSessionInfo:
    ...
    def test_runtime_resolution_failure_doesnt_crash(self, runner, tmp_path):
        ...
        with patch("harness.gateway.run._hermes_home", tmp_path), \
             ...
             patch("harness.gateway.run._resolve_runtime_agent_kwargs", side_effect=RuntimeError("no creds")):
            info = runner._format_session_info()
        assert "4K" in info
        assert "config" in info
