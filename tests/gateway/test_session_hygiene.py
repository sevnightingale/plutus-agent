...
@pytest.mark.asyncio
async def test_history_cost_ceiling_fires_below_percent_threshold(monkeypatch, tmp_path):
    """compression.max_history_tokens compresses on ABSOLUTE history size —
    the % threshold never fires on a large-window model, which is how the
    Telegram main session came to replay ~185k tokens per wake for a day
    at real money (2026-08-31)."""
    fake_dotenv = types.ModuleType("dotenv")
    fake_dotenv.load_dotenv = lambda *args, **kwargs: None
    monkeypatch.setitem(sys.modules, "dotenv", fake_dotenv)
    ...
    # ~10k tokens of history: far below 85% of a 1M-token window, above the
    # 5k absolute ceiling.
    runner.session_store.load_transcript.return_value = _make_history(
        20, content_size=2_000)
    runner.session_store.has_any_sessions.return_value = True
    runner.session_store.rewrite_transcript = MagicMock()
    runner.session_store.append_to_transcript = MagicMock()
    runner._running_agents = {}
    ...
    # Control: same history with the ceiling unset must NOT compress —
    # ~10k tokens is nowhere near 85% of a 1M window.
    (tmp_path / "config.yaml").write_text(
        "compression:\n  enabled: true\n", encoding="utf-8")
