...
class TestDeliverSyntheticMessage:
    @pytest.mark.asyncio
    async def test_routes_through_adapter_handle_message(self):
        runner, _ = _make_runner()
        adapter = _make_adapter()

        # Platform key for the adapters dict — same MagicMock instance passed
        # in via deliver_synthetic_message must equal the dict key.
        platform_obj = MagicMock(value="telegram")
        runner.adapters[platform_obj] = adapter

        await runner.deliver_synthetic_message(
            platform=platform_obj,
            chat_id="123",
            text="[SYSTEM TICK] run heartbeat",
            kind="cron:plutus-heartbeat",
            user_id="user1",
            user_name="sev",
        )

        adapter.handle_message.assert_called_once()
        synth_event = adapter.handle_message.call_args[0][0]
        assert isinstance(synth_event, MessageEvent)
        assert synth_event.synthetic_kind == "cron:plutus-heartbeat"
        assert synth_event.internal is True
        assert synth_event.text == "[SYSTEM TICK] run heartbeat"
        assert synth_event.source.chat_id == "123"
        assert synth_event.source.platform == platform_obj

    @pytest.mark.asyncio
    async def test_returns_none_when_adapter_missing(self):
        runner, _ = _make_runner()
        platform_obj = MagicMock(value="telegram")
        # No adapter registered
        result = await runner.deliver_synthetic_message(
            platform=platform_obj,
            chat_id="123",
            text="anything",
            kind="cron:foo",
    ...
    @pytest.mark.asyncio
    async def test_returns_none_when_chat_id_empty(self):
        runner, _ = _make_runner()
        adapter = _make_adapter()
        platform_obj = MagicMock(value="telegram")
        runner.adapters[platform_obj] = adapter
