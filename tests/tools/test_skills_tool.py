...
from harness.tools.skills_tool import (
...
class TestRequiredEnvironmentVariablesNormalization:
    ...
    def test_empty_env_file_value_is_treated_as_missing(self, monkeypatch):
        ...
        from harness.tools.skills_tool import _is_env_var_persisted
...
class TestGetCategoryFromPath:
    ...
    def test_outside_skills_dir(self, tmp_path):
        with patch("harness.tools.skills_tool.SKILLS_DIR", tmp_path / "skills"):
...
class TestFindAllSkills:
    ...
    def test_nonexistent_directory(self, tmp_path):
        with patch("harness.tools.skills_tool.SKILLS_DIR", tmp_path / "nope"):
            skills = _find_all_skills()
        assert skills == []
    ...
    def test_finds_skills_in_symlinked_category_dir(self, tmp_path):
        ...
        with patch("harness.tools.skills_tool.SKILLS_DIR", skills_root):
            skills = _find_all_skills()
...
class TestSkillsList:
    def test_empty_creates_directory(self, tmp_path):
        ...
        with patch("harness.tools.skills_tool.SKILLS_DIR", skills_dir):
            raw = skills_list()
...
class TestSkillView:
    ...
    def test_view_disabled_skill_blocked(self, tmp_path):
        ...
        with (
            patch("harness.tools.skills_tool.SKILLS_DIR", tmp_path),
            patch(
                "harness.tools.skills_tool._is_skill_disabled",
                return_value=True,
            ),
        ):
...
class TestFindAllSkillsPlatformFiltering:
    ...
    def test_excludes_incompatible_platform(self, tmp_path):
        with (
            patch("harness.tools.skills_tool.SKILLS_DIR", tmp_path),
            patch("harness.agent.skill_utils.sys") as mock_sys,
        ):
            mock_sys.platform = "linux"
