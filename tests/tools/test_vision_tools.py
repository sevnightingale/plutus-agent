...
from harness.tools.vision_tools import (
    _validate_image_url,
...
class TestValidateImageUrl:
    ...
    def test_valid_https_url(self):
        with patch("harness.tools.url_safety.socket.getaddrinfo", return_value=[
...
class TestHandleVisionAnalyze:
    ...
    def test_returns_awaitable(self):
        ...
        with patch(
            "harness.tools.vision_tools.vision_analyze_tool", new_callable=AsyncMock
        ) as mock_tool:
...
class TestErrorLoggingExcInfo:
    ...
    async def test_download_failure_logs_exc_info(self, tmp_path, caplog):
        ...
        from harness.tools.vision_tools import _download_image

        with patch("harness.tools.vision_tools.httpx.AsyncClient") as mock_client_cls:
            ...
            with (
                caplog.at_level(logging.ERROR, logger="harness.tools.vision_tools"),
    ...
    async def test_analysis_error_logs_exc_info(self, caplog):
        ...
        with (
            patch("harness.tools.vision_tools._validate_image_url", return_value=True),
            patch(
                "harness.tools.vision_tools._download_image",
    ...
    async def test_cleanup_error_logs_exc_info(self, tmp_path, caplog):
        ...
        with (
            patch("harness.tools.vision_tools._validate_image_url", return_value=True),
            patch("harness.tools.vision_tools._download_image", side_effect=fake_download),
            patch(
                "harness.tools.vision_tools._image_to_base64_data_url",
                return_value="data:image/jpeg;base64,abc",
            ),
            caplog.at_level(logging.WARNING, logger="harness.tools.vision_tools"),
        ):
            ...
            with (
                patch("harness.tools.vision_tools.async_call_llm", new_callable=AsyncMock, return_value=mock_response),
            ):
...
class TestVisionConfig:
    ...
    async def test_vision_uses_configured_temperature_and_timeout(self, tmp_path):
        ...
        with (
            patch("harness.cli.config.load_config", return_value={
            ...
            patch(
                "harness.tools.vision_tools.async_call_llm",
                new_callable=AsyncMock,
    ...
    async def test_vision_defaults_temperature_when_config_omits_it(self, tmp_path):
        ...
        with (
            patch("harness.cli.config.load_config", return_value={"auxiliary": {"vision": {}}}),
            patch(
                "harness.tools.vision_tools._image_to_base64_data_url",
                return_value="data:image/png;base64,abc",
            ),
...
class TestVisionSafetyGuards:
    ...
    async def test_local_non_image_file_rejected_before_llm_call(self, tmp_path):
        ...
        with patch("harness.tools.vision_tools.async_call_llm", new_callable=AsyncMock) as mock_llm:
    ...
    async def test_blocked_remote_url_short_circuits_before_download(self):
        ...
        with (
            patch("harness.tools.vision_tools.check_website_access", return_value=blocked),
            patch("harness.tools.vision_tools._validate_image_url", return_value=True),
            patch("harness.tools.vision_tools._download_image", new_callable=AsyncMock) as mock_download,
        ):
    ...
    async def test_download_blocks_redirected_final_url(self, tmp_path):
        ...
        with (
            patch("harness.tools.vision_tools.check_website_access", side_effect=fake_check),
            patch("harness.tools.vision_tools.httpx.AsyncClient") as mock_client_cls,
...
class TestBase64SizeLimit:
    ...
    async def test_oversized_image_rejected_before_api_call(self, tmp_path):
        ...
        with patch("harness.tools.vision_tools._MAX_BASE64_BYTES", 1000), \
             patch("harness.tools.vision_tools.async_call_llm", new_callable=AsyncMock) as mock_llm:
...
class TestResizeImageForVision:
    ...
    def test_no_pillow_returns_original(self, tmp_path):
        ...
        with patch("harness.tools.vision_tools._image_to_base64_data_url") as mock_b64:
