...
class TestForegroundTimeoutCap:
    ...
    def test_foreground_timeout_rejected_above_max(self):
        ...
        from harness.tools.terminal_tool import terminal_tool, FOREGROUND_MAX_TIMEOUT

        with patch("harness.tools.terminal_tool._get_env_config", return_value=_make_env_config()), \
             patch("harness.tools.terminal_tool._start_cleanup_thread"):
    ...
    def test_foreground_rejects_shell_level_background_wrappers(self):
        ...
        from harness.tools.terminal_tool import terminal_tool
    ...
    def test_foreground_allows_help_variant_for_server_command(self):
        ...
        with patch("harness.tools.terminal_tool._get_env_config", return_value=_make_env_config()), \
            ...
            with patch("harness.tools.terminal_tool._active_environments", {"default": mock_env}), \
                 patch("harness.tools.terminal_tool._last_activity", {"default": 0}), \
                 patch("harness.tools.terminal_tool._check_all_guards", return_value={"approved": True}):
    ...
    def test_config_default_above_cap_not_rejected(self):
        ...
        with patch("harness.tools.terminal_tool._get_env_config",
    ...
    def test_background_not_rejected(self):
        ...
        with patch("harness.tools.terminal_tool._get_env_config", return_value=_make_env_config()), \
            ...
            with patch("harness.tools.terminal_tool._active_environments", {"default": mock_env}), \
                 patch("harness.tools.terminal_tool._last_activity", {"default": 0}), \
                 patch("harness.tools.terminal_tool._check_all_guards", return_value={"approved": True}), \
                 patch("harness.tools.process_registry.process_registry", mock_registry), \
                 patch("harness.tools.approval.get_current_session_key", return_value=""):
                result = json.loads(terminal_tool(
...
class TestForegroundMaxTimeoutConstant:
    ...
    def test_default_value_is_600(self):
        ...
        from harness.tools.terminal_tool import FOREGROUND_MAX_TIMEOUT
        assert FOREGROUND_MAX_TIMEOUT == 600

    def test_schema_mentions_max(self):
        """Tool schema description should mention the max timeout."""
        from harness.tools.terminal_tool import TERMINAL_SCHEMA, FOREGROUND_MAX_TIMEOUT
