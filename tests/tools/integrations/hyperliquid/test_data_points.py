...
from tools.core import data_point_registry, account_registry, venue_registry, alert_registry
from tools.integrations.hyperliquid import _client
...
def _import_integration():
    ...
    import tools.integrations.hyperliquid as hl_pkg
...
def test_hl_price_live():
    _import_integration()
    from tools.integrations.hyperliquid.data_points import hl_price
    res = hl_price("BTC")
...
def test_hl_candles_live():
    _import_integration()
    from tools.integrations.hyperliquid.data_points import hl_candles
...
def test_hl_orderbook_live():
    _import_integration()
    from tools.integrations.hyperliquid.data_points import hl_orderbook
...
def test_hl_funding_and_oi_live():
    _import_integration()
    from tools.integrations.hyperliquid.data_points import hl_funding_and_oi
...
def test_hl_universe_live():
    _import_integration()
    from tools.integrations.hyperliquid.data_points import hl_universe
    res = hl_universe()
...
def test_hl_holdings_loud_fails_without_address(monkeypatch):
    ...
    from tools.integrations.hyperliquid.data_points import hl_holdings
...
def test_hl_total_equity_loud_fails_without_address(monkeypatch):
    _import_integration()
    monkeypatch.delenv("HL_PUBLIC_ADDRESS", raising=False)
    from tools.integrations.hyperliquid.data_points import hl_total_equity
    with pytest.raises(_client.HLConfigError):
