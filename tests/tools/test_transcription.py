...
class TestValidateAudioFile:
    ...
    def test_too_large(self, tmp_path):
        ...
        f = tmp_path / "big.ogg"
        f.write_bytes(b"x")
        from harness.tools.transcription_tools import _validate_audio_file, MAX_FILE_SIZE
        real_stat = f.stat()
...
class TestTranscribeAudio:
    ...
    def test_dispatches_to_local(self, tmp_path):
        ...
        with patch("harness.tools.transcription_tools._load_stt_config", return_value={"provider": "local"}), \
             patch("harness.tools.transcription_tools._get_provider", return_value="local"), \
             patch("harness.tools.transcription_tools._transcribe_local", return_value={"success": True, "transcript": "hi"}) as mock_local:
    ...
    def test_dispatches_to_openai(self, tmp_path):
        ...
        with patch("harness.tools.transcription_tools._load_stt_config", return_value={"provider": "openai"}), \
             ...
             patch("harness.tools.transcription_tools._transcribe_openai", return_value={"success": True, "transcript": "hi"}) as mock_openai:
            from harness.tools.transcription_tools import transcribe_audio
            result = transcribe_audio(str(audio_file))
    ...
    def test_disabled_config_returns_disabled_error(self, tmp_path):
        ...
        with patch("harness.tools.transcription_tools._load_stt_config", return_value={"enabled": False}), \
             patch("harness.tools.transcription_tools._get_provider", return_value="none"):
            from harness.tools.transcription_tools import transcribe_audio
            result = transcribe_audio(str(audio_file))
...
class TestNormalizeLocalModel:
    ...
    def test_openai_model_name_maps_to_default(self):
        from harness.tools.transcription_tools import _normalize_local_model, DEFAULT_LOCAL_MODEL
    ...
    def test_valid_local_model_preserved(self):
        from harness.tools.transcription_tools import _normalize_local_model
    ...
    def test_warning_emitted_for_cloud_model(self, caplog):
        import logging
        from harness.tools.transcription_tools import _normalize_local_model
        with caplog.at_level(logging.WARNING, logger="harness.tools.transcription_tools"):
    ...
    def test_local_transcribe_normalises_model(self):
        ...
        try:
            ...
            with patch("harness.tools.transcription_tools._HAS_FASTER_WHISPER", True), \
                 patch("harness.tools.transcription_tools._load_stt_config", return_value={
                     "enabled": True,
                     "provider": "local",
