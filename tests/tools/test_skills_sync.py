...
from harness.tools.skills_sync import (
    _get_bundled_dir,
    _read_manifest,
    _read_skill_name,
    _write_manifest,
...
class TestReadWriteManifest:
    def test_read_missing_manifest(self, tmp_path):
        with patch(
            "harness.tools.skills_sync.MANIFEST_FILE",
    ...
    def test_write_and_read_roundtrip_v2(self, tmp_path):
        ...
        with patch("harness.tools.skills_sync.MANIFEST_FILE", manifest_file):
...
class TestSyncSkills:
    ...
    def _patches(self, bundled, skills_dir, manifest_file):
        ...
        stack = ExitStack()
        stack.enter_context(patch("harness.tools.skills_sync._get_bundled_dir", return_value=bundled))
        stack.enter_context(patch("harness.tools.skills_sync.SKILLS_DIR", skills_dir))
        stack.enter_context(patch("harness.tools.skills_sync.MANIFEST_FILE", manifest_file))
        return stack
    ...
    def test_nonexistent_bundled_dir(self, tmp_path):
        with patch("harness.tools.skills_sync._get_bundled_dir", return_value=tmp_path / "nope"):
            result = sync_skills(quiet=True)
        assert result == {
