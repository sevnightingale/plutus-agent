...
import harness.tools.tirith_security as _tirith_mod
from harness.tools.tirith_security import check_command_security, ensure_installed
...
class TestExitCodeMapping:
    @patch("harness.tools.tirith_security.subprocess.run")
    @patch("harness.tools.tirith_security._load_security_config")
...
class TestPathExpansion:
    def test_tilde_expanded_in_resolve(self):
        ...
        from harness.tools.tirith_security import _resolve_tirith_path
        _tirith_mod._resolved_path = None
...
class TestEnsureInstalled:
    ...
    @patch("harness.tools.tirith_security.shutil.which", return_value="/usr/local/bin/tirith")
    @patch("harness.tools.tirith_security._load_security_config")
    ...
    def test_not_found_returns_none(self, mock_cfg):
        mock_cfg.return_value = {"tirith_enabled": True, "tirith_path": "tirith",
                                 "tirith_timeout": 5, "tirith_fail_open": True}
        _tirith_mod._resolved_path = None
        with patch("harness.tools.tirith_security.shutil.which", return_value=None), \
             patch("harness.tools.tirith_security._hermes_bin_dir", return_value="/nonexistent"), \
             patch("harness.tools.tirith_security._is_install_failed_on_disk", return_value=False), \
             patch("harness.tools.tirith_security.threading.Thread") as MockThread:
...
class TestFailedDownloadCaching:
    @patch("harness.tools.tirith_security._mark_install_failed")
    @patch("harness.tools.tirith_security._is_install_failed_on_disk", return_value=False)
    @patch("harness.tools.tirith_security._install_tirith", return_value=(None, "download_failed"))
    @patch("harness.tools.tirith_security.shutil.which", return_value=None)
    def test_failed_install_cached_no_retry(self, mock_which, mock_install,
        ...
        from harness.tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        _tirith_mod._resolved_path = None
...
class TestExplicitPathNoAutoDownload:
    @patch("harness.tools.tirith_security._install_tirith")
    @patch("harness.tools.tirith_security.shutil.which", return_value=None)
    ...
    @patch("harness.tools.tirith_security._mark_install_failed")
    @patch("harness.tools.tirith_security._is_install_failed_on_disk", return_value=False)
    @patch("harness.tools.tirith_security._install_tirith", return_value=("/auto/tirith", ""))
    @patch("harness.tools.tirith_security.shutil.which", return_value=None)
...
class TestCosignVerification:
    @patch("harness.tools.tirith_security.subprocess.run")
    @patch("harness.tools.tirith_security.shutil.which", return_value="/usr/bin/cosign")
    def test_cosign_pass(self, mock_which, mock_run):
        """cosign verify-blob exits 0 → returns True."""
        from harness.tools.tirith_security import _verify_cosign
    ...
    @patch("harness.tools.tirith_security._verify_cosign", return_value=False)
    @patch("harness.tools.tirith_security.shutil.which", return_value="/usr/local/bin/cosign")
    @patch("harness.tools.tirith_security._download_file")
    @patch("harness.tools.tirith_security._detect_target", return_value="aarch64-apple-darwin")
    def test_install_aborts_on_cosign_rejection(self, mock_target, mock_dl,
        ...
        from harness.tools.tirith_security import _install_tirith
        path, reason = _install_tirith()
        assert path is None
        assert reason == "cosign_verification_failed"

    @patch("harness.tools.tirith_security.tarfile.open")
    @patch("harness.tools.tirith_security._verify_checksum", return_value=True)
    @patch("harness.tools.tirith_security.shutil.which", return_value=None)
    @patch("harness.tools.tirith_security._download_file")
    @patch("harness.tools.tirith_security._detect_target", return_value="aarch64-apple-darwin")
    ...
    @patch("harness.tools.tirith_security.tarfile.open")
    @patch("harness.tools.tirith_security._verify_checksum", return_value=True)
    @patch("harness.tools.tirith_security._verify_cosign", return_value=None)
    @patch("harness.tools.tirith_security.shutil.which", return_value="/usr/local/bin/cosign")
    @patch("harness.tools.tirith_security._download_file")
    @patch("harness.tools.tirith_security._detect_target", return_value="aarch64-apple-darwin")
    ...
    @patch("harness.tools.tirith_security.tarfile.open")
    @patch("harness.tools.tirith_security._verify_checksum", return_value=True)
    @patch("harness.tools.tirith_security._verify_cosign", return_value=True)
    @patch("harness.tools.tirith_security.shutil.which", return_value="/usr/local/bin/cosign")
    @patch("harness.tools.tirith_security._download_file")
    @patch("harness.tools.tirith_security._detect_target", return_value="aarch64-apple-darwin")
...
class TestBackgroundInstall:
    def test_ensure_installed_non_blocking(self):
        ...
        with patch("harness.tools.tirith_security._load_security_config",
    ...
    def test_ensure_installed_skips_on_disk_marker(self):
        ...
        with patch("harness.tools.tirith_security._load_security_config",
             ...
             patch("harness.tools.tirith_security.shutil.which", return_value=None), \
             patch("harness.tools.tirith_security._hermes_bin_dir", return_value="/nonexistent"), \
             patch("harness.tools.tirith_security._read_failure_reason", return_value="download_failed"), \
             patch("harness.tools.tirith_security._is_install_failed_on_disk", return_value=True):
...
class TestDiskFailureMarker:
    def test_mark_and_check(self):
        ...
        with patch("harness.tools.tirith_security._failure_marker_path", return_value=marker):
            from harness.tools.tirith_security import (
    ...
    def test_expired_marker_ignored(self):
        ...
        marker = os.path.join(tmpdir, ".tirith-install-failed")
        with patch("harness.tools.tirith_security._failure_marker_path", return_value=marker):
            from harness.tools.tirith_security import _mark_install_failed, _is_install_failed_on_disk
            _mark_install_failed("download_failed")
    ...
    def test_cosign_missing_marker_clears_when_cosign_appears(self):
        ...
        with patch("harness.tools.tirith_security._failure_marker_path", return_value=marker):
            ...
            with patch("harness.tools.tirith_security.shutil.which", return_value="/usr/local/bin/cosign"):
                assert not _is_install_failed_on_disk()
    ...
    @patch("harness.tools.tirith_security._mark_install_failed")
    @patch("harness.tools.tirith_security._is_install_failed_on_disk", return_value=False)
    @patch("harness.tools.tirith_security._install_tirith", return_value=(None, "cosign_missing"))
    @patch("harness.tools.tirith_security.shutil.which", return_value=None)
    ...
    @patch("harness.tools.tirith_security._clear_install_failed")
    @patch("harness.tools.tirith_security._is_install_failed_on_disk", return_value=False)
    @patch("harness.tools.tirith_security._install_tirith", return_value=("/installed/tirith", ""))
    @patch("harness.tools.tirith_security.shutil.which", return_value=None)
    ...
    def test_sync_resolve_skips_install_on_disk_marker(self):
        ...
        with patch("harness.tools.tirith_security.shutil.which", return_value=None), \
             ...
             patch("harness.tools.tirith_security._read_failure_reason", return_value="download_failed"), \
             patch("harness.tools.tirith_security._is_install_failed_on_disk", return_value=True), \
             patch("harness.tools.tirith_security._install_tirith") as mock_install:
            _resolve_tirith_path("tirith")
            mock_install.assert_not_called()
            assert _tirith_mod._resolved_path is _INSTALL_FAILED
            assert _tirith_mod._install_failure_reason == "download_failed"
    ...
    def test_install_failed_still_checks_local_paths(self):
        ...
        with patch("harness.tools.tirith_security.shutil.which", return_value="/usr/local/bin/tirith"), \
             patch("harness.tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == "/usr/local/bin/tirith"
    ...
    def test_install_failed_recovers_from_hermes_bin(self):
        ...
        with patch("harness.tools.tirith_security.shutil.which", return_value=None), \
             patch("harness.tools.tirith_security._hermes_bin_dir", return_value=tmpdir), \
             patch("harness.tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
    ...
    def test_cosign_missing_disk_marker_allows_retry(self):
        ...
        with patch("harness.tools.tirith_security.shutil.which", return_value=None), \
             patch("harness.tools.tirith_security._hermes_bin_dir", return_value="/nonexistent"), \
             patch("harness.tools.tirith_security._is_install_failed_on_disk", return_value=False), \
             patch("harness.tools.tirith_security._install_tirith", return_value=("/new/tirith", "")) as mock_install, \
             patch("harness.tools.tirith_security._clear_install_failed"):
            result = _resolve_tirith_path("tirith")
    ...
    def test_in_memory_cosign_missing_retries_when_cosign_appears(self):
        ...
        with patch("harness.tools.tirith_security.shutil.which", side_effect=_which_side_effect), \
             patch("harness.tools.tirith_security._hermes_bin_dir", return_value="/nonexistent"), \
             patch("harness.tools.tirith_security._is_install_failed_on_disk", return_value=False), \
             patch("harness.tools.tirith_security._install_tirith", return_value=("/new/tirith", "")) as mock_install, \
             patch("harness.tools.tirith_security._clear_install_failed"):
    ...
    def test_disk_marker_reason_preserved_in_memory(self):
        ...
        with patch("harness.tools.tirith_security.shutil.which", return_value=None), \
             patch("harness.tools.tirith_security._hermes_bin_dir", return_value="/nonexistent"), \
             patch("harness.tools.tirith_security._read_failure_reason", return_value="cosign_missing"), \
             patch("harness.tools.tirith_security._is_install_failed_on_disk", return_value=True):
            _resolve_tirith_path("tirith")
            assert _tirith_mod._resolved_path is _INSTALL_FAILED
...
class TestHermesHomeIsolation:
    def test_hermes_bin_dir_respects_hermes_home(self):
        ...
        from harness.tools.tirith_security import _hermes_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
    ...
    def test_failure_marker_respects_hermes_home(self):
        ...
        from harness.tools.tirith_security import _failure_marker_path
    ...
    def test_get_hermes_home_fallback(self):
        ...
        from harness.tools.tirith_security import _get_hermes_home
