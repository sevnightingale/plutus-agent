...
class TestResolveCdpOverride:
    def test_keeps_full_devtools_websocket_url(self):
        from harness.tools.browser_tool import _resolve_cdp_override
    ...
    def test_resolves_http_discovery_endpoint_to_websocket(self):
        ...
        with patch("harness.tools.browser_tool.requests.get", return_value=response) as mock_get:
    ...
    def test_falls_back_to_raw_url_when_discovery_fails(self):
        ...
        with patch("harness.tools.browser_tool.requests.get", side_effect=RuntimeError("boom")):
            assert _resolve_cdp_override(HTTP_URL) == HTTP_URL

    def test_normalizes_provider_returned_http_cdp_url_when_creating_session(self, monkeypatch):
        import harness.tools.browser_tool as browser_tool
...
class TestGetCdpOverride:
    ...
    def test_uses_config_browser_cdp_url_when_env_missing(self, monkeypatch):
        ...
        with patch("harness.cli.config.read_raw_config", return_value={"browser": {"cdp_url": HTTP_URL}}), \
             patch("harness.tools.browser_tool.requests.get", return_value=response) as mock_get:
            resolved = browser_tool._get_cdp_override()
