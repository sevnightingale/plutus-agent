...
class TestGenerateMistralTts:
    def test_missing_api_key_raises_value_error(self, tmp_path, mock_mistral_module):
        from harness.tools.tts_tool import _generate_mistral_tts
    ...
    def test_default_voice_id_when_absent(
        ...
        from harness.tools.tts_tool import DEFAULT_MISTRAL_TTS_VOICE_ID, _generate_mistral_tts
    ...
    def test_default_model_used(self, tmp_path, mock_mistral_module, monkeypatch):
        from harness.tools.tts_tool import DEFAULT_MISTRAL_TTS_MODEL, _generate_mistral_tts
...
class TestTtsDispatcherMistral:
    def test_dispatcher_routes_to_mistral(
        ...
        with patch("harness.tools.tts_tool._load_tts_config", return_value={"provider": "mistral"}):
    ...
    def test_dispatcher_returns_error_when_sdk_not_installed(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MISTRAL_API_KEY", "test-key")
        with patch(
            "harness.tools.tts_tool._import_mistral_client", side_effect=ImportError("no module")
        ), patch("harness.tools.tts_tool._load_tts_config", return_value={"provider": "mistral"}):
            result = json.loads(
...
class TestCheckTtsRequirementsMistral:
    def test_mistral_sdk_and_key_returns_true(self, mock_mistral_module, monkeypatch):
        ...
        monkeypatch.setenv("MISTRAL_API_KEY", "test-key")
        with patch("harness.tools.tts_tool._import_edge_tts", side_effect=ImportError), \
             patch("harness.tools.tts_tool._import_elevenlabs", side_effect=ImportError), \
             patch("harness.tools.tts_tool._import_openai_client", side_effect=ImportError), \
             patch("harness.tools.tts_tool._check_neutts_available", return_value=False):
