...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        import harness.tools.web_tools; import harness.tools as tools
        ...
        self._managed_patchers = [
            patch("harness.tools.web_tools.managed_nous_tools_enabled", return_value=True),
            patch("harness.tools.managed_tool_gateway.managed_nous_tools_enabled", return_value=True),
        ]
    ...
    def test_no_config_raises_with_helpful_message(self):
        ...
        with patch("harness.tools.web_tools.Firecrawl"):
            with patch("harness.tools.web_tools._read_nous_access_token", return_value=None):
                from harness.tools.web_tools import _get_firecrawl_client
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "nousresearch.com"}):
            with patch("harness.tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("harness.tools.web_tools.Firecrawl") as mock_fc:
                    from harness.tools.web_tools import _get_firecrawl_client
    ...
    def test_nous_auth_token_respects_hermes_home_override(self, tmp_path):
        """Auth lookup should read from HERMES_HOME/auth.json, not ~/.plutus-agent/auth.json."""
    ...
    def test_check_auxiliary_model_re_resolves_backend_each_call(self):
        ...
        with patch(
            "harness.tools.web_tools.get_async_text_auxiliary_client",
    ...
    async def test_summarizer_re_resolves_backend_after_initial_unavailable_state(self):
        ...
        with patch(
            "harness.tools.web_tools._resolve_web_extract_auxiliary",
            side_effect=[(None, None, {}), (MagicMock(base_url="https://api.openrouter.ai/v1"), "test-model", {})],
        ), patch(
            "harness.tools.web_tools.async_call_llm",
...
class TestBackendSelection:
    ...
    def test_config_parallel(self):
        ...
        from harness.tools.web_tools import _get_backend
        with patch("harness.tools.web_tools._load_web_config", return_value={"backend": "parallel"}):
            assert _get_backend() == "parallel"

    def test_config_exa(self):
        """web.backend=exa in config → 'exa' regardless of other keys."""
        from harness.tools.web_tools import _get_backend
        with patch("harness.tools.web_tools._load_web_config", return_value={"backend": "exa"}), \
    ...
    def test_config_firecrawl(self):
        ...
        from harness.tools.web_tools import _get_backend
        with patch("harness.tools.web_tools._load_web_config", return_value={"backend": "firecrawl"}), \
             patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
    ...
    def test_config_tavily(self):
        ...
        from harness.tools.web_tools import _get_backend
        with patch("harness.tools.web_tools._load_web_config", return_value={"backend": "tavily"}):
    ...
    def test_fallback_parallel_only_key(self):
        ...
        from harness.tools.web_tools import _get_backend
        with patch("harness.tools.web_tools._load_web_config", return_value={}), \
             patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
            assert _get_backend() == "parallel"
    ...
    def test_invalid_config_falls_through_to_fallback(self):
        ...
        from harness.tools.web_tools import _get_backend
        with patch("harness.tools.web_tools._load_web_config", return_value={"backend": "nonexistent"}), \
             patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
            assert _get_backend() == "parallel"
...
class TestParallelClientConfig:
    ...
    def test_creates_client_with_key(self):
        ...
        with patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
            from harness.tools.web_tools import _get_parallel_client
...
class TestWebSearchErrorHandling:
    ...
    def test_search_error_response_does_not_expose_diagnostics(self):
        ...
        with patch("harness.tools.web_tools._get_backend", return_value="firecrawl"), \
             patch("harness.tools.web_tools._get_firecrawl_client", return_value=firecrawl_client), \
...
class TestCheckWebApiKey:
    ...
    def test_parallel_key_only(self):
        with patch.dict(os.environ, {"PARALLEL_API_KEY": "test-key"}):
            from harness.tools.web_tools import check_web_api_key
...
def test_web_requires_env_includes_exa_key():
    from harness.tools.web_tools import _web_requires_env
