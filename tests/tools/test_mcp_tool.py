...
class TestLoadMCPConfig:
    def test_no_config_returns_empty(self):
        ...
        with patch("harness.cli.config.load_config", return_value={"model": "test"}):
            from harness.tools.mcp_tool import _load_mcp_config
    ...
    def test_valid_config_parsed(self):
        ...
        with patch("harness.cli.config.load_config", return_value={"mcp_servers": servers}):
            from harness.tools.mcp_tool import _load_mcp_config
            result = _load_mcp_config()
    ...
    def test_mcp_servers_not_dict_returns_empty(self):
        ...
        with patch("harness.cli.config.load_config", return_value={"mcp_servers": "invalid"}):
            from harness.tools.mcp_tool import _load_mcp_config
            result = _load_mcp_config()
            assert result == {}
...
class TestSchemaConversion:
    def test_converts_mcp_tool_to_hermes_schema(self):
        from harness.tools.mcp_tool import _convert_mcp_schema
...
class TestCheckFunction:
    def test_disconnected_returns_false(self):
        from harness.tools.mcp_tool import _make_check_fn, _servers
...
class TestToolHandler:
    ...
    def _patch_mcp_loop(self, coro_side_effect=None):
        ...
        if coro_side_effect:
            return patch("harness.tools.mcp_tool._run_on_mcp_loop", side_effect=coro_side_effect)
        return patch("harness.tools.mcp_tool._run_on_mcp_loop", side_effect=fake_run)

    def test_successful_call(self):
        from harness.tools.mcp_tool import _make_tool_handler, _servers
    ...
    def test_interrupted_call_returns_interrupted_error(self):
        ...
        try:
            ...
            with patch(
                "harness.tools.mcp_tool._run_on_mcp_loop",
...
class TestDiscoverAndRegister:
    def test_tools_registered_in_registry(self):
        ...
        from harness.tools.mcp_tool import _discover_and_register_server, _servers, MCPServerTask
        ...
        with patch("harness.tools.mcp_tool._connect_server", side_effect=fake_connect), \
             patch("harness.tools.registry.registry", mock_registry):
...
class TestMCPServerTask:
    ...
    def _mock_stdio_and_session(self, session):
        ...
        return (
            ...
            patch("harness.tools.mcp_tool.ClientSession", return_value=mock_cs_cm),
    ...
    def test_start_connects_and_discovers_tools(self):
        ...
        async def _test():
            with patch("harness.tools.mcp_tool.StdioServerParameters"), p_stdio, p_cs:
    ...
    def test_empty_env_gets_safe_defaults(self):
        ...
        async def _test():
            with patch("harness.tools.mcp_tool.StdioServerParameters") as mock_params, \
                 p_stdio, p_cs, \
...
class TestToolsetInjection:
    def test_mcp_tools_resolve_through_server_aliases(self):
        ...
        with patch("harness.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("harness.tools.mcp_tool._servers", fresh_servers), \
             patch("harness.tools.mcp_tool._load_mcp_config", return_value=fake_config), \
    ...
    def test_server_toolset_skips_builtin_collision(self):
        ...
        with patch("harness.tools.mcp_tool._MCP_AVAILABLE", True), \
             ...
             patch("harness.tools.mcp_tool._connect_server", side_effect=fake_connect), \
             patch("harness.tools.registry.registry", mock_registry), \
             patch("harness.toolsets.TOOLSETS", fake_toolsets):
            from harness.tools.mcp_tool import discover_mcp_tools
            discover_mcp_tools()
    ...
    def test_server_connection_failure_skipped(self):
        ...
        with patch("harness.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("harness.tools.mcp_tool._servers", fresh_servers), \
             patch("harness.tools.mcp_tool._load_mcp_config", return_value=fake_config), \
             patch("harness.tools.mcp_tool._connect_server", side_effect=flaky_connect), \
             patch("harness.toolsets.TOOLSETS", fake_toolsets):
            from harness.tools.mcp_tool import discover_mcp_tools
            result = discover_mcp_tools()
...
class TestGracefulFallback:
    ...
    def test_no_servers_returns_empty(self):
        ...
        with patch("harness.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("harness.tools.mcp_tool._servers", {}), \
             patch("harness.tools.mcp_tool._load_mcp_config", return_value={}):
            from harness.tools.mcp_tool import discover_mcp_tools
            result = discover_mcp_tools()
            assert result == []
...
class TestShutdown:
    ...
    def test_shutdown_deregisters_registered_tools(self):
        ...
        import harness.tools.mcp_tool as mcp_mod
        from harness.tools.mcp_tool import MCPServerTask, shutdown_mcp_servers, _servers
...
class TestBuildSafeEnv:
    ...
    def test_only_safe_vars_passed(self):
        ...
        from harness.tools.mcp_tool import _build_safe_env
...
class TestSanitizeError:
    ...
    def test_strips_github_pat(self):
        from harness.tools.mcp_tool import _sanitize_error
...
class TestHTTPConfig:
    ...
    def test_http_unavailable_raises(self):
        ...
        async def _test():
            with patch("harness.tools.mcp_tool._MCP_HTTP_AVAILABLE", False):
...
class TestConfigurableTimeouts:
    ...
    def test_default_timeout(self):
        ...
        from harness.tools.mcp_tool import MCPServerTask, _DEFAULT_TOOL_TIMEOUT
    ...
    def test_timeout_passed_to_handler(self):
        ...
        from harness.tools.mcp_tool import _make_tool_handler, _servers, MCPServerTask
        ...
        try:
            ...
            with patch("harness.tools.mcp_tool._run_on_mcp_loop") as mock_run:
                def fake_run(coro, timeout=30):
                    coro.close()
...
class TestUtilitySchemas:
    ...
    def test_builds_four_utility_schemas(self):
        from harness.tools.mcp_tool import _build_utility_schemas
...
class TestUtilityHandlers:
    ...
    def test_list_resources_success(self):
        from harness.tools.mcp_tool import _make_list_resources_handler, _servers
    ...
    def test_read_resource_success(self):
        from harness.tools.mcp_tool import _make_read_resource_handler, _servers
    ...
    def test_list_prompts_success(self):
        from harness.tools.mcp_tool import _make_list_prompts_handler, _servers
    ...
    def test_get_prompt_success(self):
        from harness.tools.mcp_tool import _make_get_prompt_handler, _servers
...
from harness.tools.mcp_tool import SamplingHandler, _safe_numeric
...
class TestSamplingCallbackText:
    ...
    def test_text_response(self):
        ...
        with patch(
            "harness.agent.auxiliary_client.call_llm",
...
class TestMCPServerTaskSamplingIntegration:
    def test_sampling_handler_created_when_enabled(self):
        ...
        from harness.tools.mcp_tool import MCPServerTask, _MCP_SAMPLING_TYPES
...
class TestDiscoveryFailedCount:
    ...
    def test_failed_server_increments_failed_count(self):
        ...
        from harness.tools.mcp_tool import discover_mcp_tools, _servers, _ensure_mcp_loop
        ...
        with patch("harness.tools.mcp_tool._load_mcp_config", return_value=fake_config), \
             patch("harness.tools.mcp_tool._discover_and_register_server", side_effect=fake_register), \
             patch("harness.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("harness.tools.mcp_tool._existing_tool_names", return_value=["mcp_good_server_tool_a"]):
            _ensure_mcp_loop()

            # Capture the logger to verify failed_count in summary
            with patch("harness.tools.mcp_tool.logger") as mock_logger:
                discover_mcp_tools()
    ...
    def test_all_servers_fail_still_prints_summary(self):
        ...
        with patch("harness.tools.mcp_tool._load_mcp_config", return_value=fake_config), \
             patch("harness.tools.mcp_tool._discover_and_register_server", side_effect=always_fail), \
             patch("harness.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("harness.tools.mcp_tool._existing_tool_names", return_value=[]):
            _ensure_mcp_loop()
    ...
    def test_ok_servers_excludes_failures(self):
        ...
        with patch("harness.tools.mcp_tool._load_mcp_config", return_value=fake_config), \
             patch("harness.tools.mcp_tool._discover_and_register_server", side_effect=selective_register), \
             patch("harness.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("harness.tools.mcp_tool._existing_tool_names", return_value=["mcp_ok1_t", "mcp_ok2_t"]):
            _ensure_mcp_loop()
...
class TestMCPSelectiveToolLoading:
    ...
    def _run_discover(self, name, tool_names, config, session=None):
        from harness.tools.registry import ToolRegistry
        from harness.tools.mcp_tool import _discover_and_register_server, _servers
        ...
        async def run():
            with patch("harness.tools.mcp_tool._connect_server", side_effect=fake_connect), \
                 patch("harness.tools.registry.registry", mock_registry), \
                 patch("harness.toolsets.create_custom_toolset"):
    ...
    def test_existing_tool_names_reflect_registered_subset(self):
        from harness.tools.mcp_tool import _existing_tool_names, _servers, _discover_and_register_server
        from harness.tools.registry import ToolRegistry
        ...
        async def run():
            with patch("harness.tools.mcp_tool._connect_server", side_effect=fake_connect), \
                 patch.dict("harness.tools.mcp_tool._servers", {}, clear=True), \
                 patch("harness.tools.registry.registry", mock_registry), \
                 patch("harness.toolsets.create_custom_toolset"):
    ...
    def test_no_toolset_created_when_everything_is_filtered_out(self):
        ...
        async def run():
            with patch("harness.tools.mcp_tool._connect_server", side_effect=fake_connect), \
                 patch("harness.tools.registry.registry", mock_registry), \
                 patch("harness.toolsets.create_custom_toolset", mock_create):
...
class TestRegistryCollisionWarning:
    ...
    def test_overwrite_different_toolset_logs_warning(self, caplog):
        ...
        with caplog.at_level(logging.ERROR, logger="harness.tools.registry"):
    ...
    def test_overwrite_same_toolset_no_warning(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="harness.tools.registry"):
            reg.register(name="my_tool", toolset="mcp-server", schema=schema, handler=handler)
...
class TestSanitizeMcpNameComponent:
    ...
    def test_hyphens_replaced(self):
        from harness.tools.mcp_tool import sanitize_mcp_name_component
    ...
    def test_slash_in_server_alias_resolution(self):
        ...
        with patch("harness.tools.registry.registry", reg):
...
class TestRegisterMcpServers:
    ...
    def test_empty_servers_returns_empty(self):
        from harness.tools.mcp_tool import register_mcp_servers
    ...
    def test_skips_already_connected_servers(self):
        from harness.tools.mcp_tool import register_mcp_servers, _servers
        ...
        try:
            with patch("harness.tools.mcp_tool._MCP_AVAILABLE", True), \
                 patch("harness.tools.mcp_tool._existing_tool_names", return_value=["mcp_existing_tool"]):
    ...
    def test_connects_new_servers(self):
        from harness.tools.mcp_tool import register_mcp_servers, _servers, _ensure_mcp_loop
        ...
        with patch("harness.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("harness.tools.mcp_tool._discover_and_register_server", side_effect=fake_register), \
             patch("harness.tools.mcp_tool._existing_tool_names", return_value=["mcp_my_server_tool1"]):
            _ensure_mcp_loop()
    ...
    def test_logs_summary_on_success(self):
        ...
        with patch("harness.tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("harness.tools.mcp_tool._discover_and_register_server", side_effect=fake_register), \
             patch("harness.tools.mcp_tool._existing_tool_names", return_value=["mcp_srv_t1", "mcp_srv_t2"]):
            _ensure_mcp_loop()
